...
pub fn t_libelle(code: &str, lang: &str) -> Option<&'static str> {
    ...
    let row: [&str; 5] = match code {
        ...
        "MT_SSC" => ["Social Security Contributions (Class 1)", "Sozialversicherungsbeiträge (Klasse 1)", "Socialezekerheidsbijdragen (klasse 1)", "Contributi di sicurezza sociale (classe 1)", "Cotizaciones a la seguridad social (clase 1)"],
        ...
        "NL_WHK" => ["Whk — return-to-work fund (WGA + Ziektewet)", "Whk — Wiedereingliederungskasse (WGA + Ziektewet)", "Whk — Werkhervattingskas (WGA + Ziektewet)", "Whk — cassa di reinserimento lavorativo (WGA + Ziektewet)", "Whk — fondo de reincorporación laboral (WGA + Ziektewet)"],
...
pub fn t_explication(key: &str, lang: &str) -> Option<&'static str> {
    ...
    let row: [&str; 5] = match key {
        ...
        "NL_LOONHEFFING" => [
            "Loonheffing {annee} (employee below AOW age) — monthly withholding at source.\n\nEstimated annual income: {rev} €\n\nBox 1 scale:\n• band 1 (0 → {b1p} €): {b1t} % (incl. national insurance premiums {pvv} %)\n• band 2 ({b1p} → {b2p} €): {b2t} %\n• band 3 (> {b2p} €): {b3t} %\n= gross tax + premiums: {box1} €/year\n\n− algemene heffingskorting (general tax credit): {ahk} €\n− arbeidskorting (labour tax credit): {ak} €\n= loonheffing: {ha} €/year / 12 = {mens} €/month\nEffective rate: {teff} %\n\nLegal basis: Wet IB 2001; Belastingplan {annee}.",
            "Loonheffing {annee} (Arbeitnehmer unter AOW-Alter) — monatlicher Quellenabzug.\n\nGeschätztes Jahreseinkommen: {rev} €\n\nTarif Box 1:\n• Stufe 1 (0 → {b1p} €): {b1t} % (davon Volksversicherungsprämien {pvv} %)\n• Stufe 2 ({b1p} → {b2p} €): {b2t} %\n• Stufe 3 (> {b2p} €): {b3t} %\n= Steuer + Prämien brutto: {box1} €/Jahr\n\n− algemene heffingskorting (allgemeiner Steuerabzug): {ahk} €\n− arbeidskorting (Arbeitsabsetzbetrag): {ak} €\n= loonheffing: {ha} €/Jahr / 12 = {mens} €/Monat\nEffektivsatz: {teff} %\n\nRechtsgrundlage: Wet IB 2001; Belastingplan {annee}.",
            "Loonheffing {annee} (werknemer onder AOW-leeftijd) — maandelijkse inhouding aan de bron.\n\nGeschat jaarinkomen: {rev} €\n\nSchijventarief box 1:\n• schijf 1 (0 → {b1p} €): {b1t} % (waarvan premies volksverzekeringen {pvv} %)\n• schijf 2 ({b1p} → {b2p} €): {b2t} %\n• schijf 3 (> {b2p} €): {b3t} %\n= bruto belasting + premies: {box1} €/jaar\n\n− algemene heffingskorting: {ahk} €\n− arbeidskorting: {ak} €\n= loonheffing: {ha} €/jaar / 12 = {mens} €/maand\nEffectief tarief: {teff} %\n\nWettelijke basis: Wet IB 2001; Belastingplan {annee}.",
            "Loonheffing {annee} (dipendente sotto l'età AOW) — ritenuta mensile alla fonte.\n\nReddito annuo stimato: {rev} €\n\nScala box 1:\n• scaglione 1 (0 → {b1p} €): {b1t} % (di cui premi volksverzekeringen {pvv} %)\n• scaglione 2 ({b1p} → {b2p} €): {b2t} %\n• scaglione 3 (> {b2p} €): {b3t} %\n= imposta + premi lordi: {box1} €/anno\n\n− algemene heffingskorting (credito generale): {ahk} €\n− arbeidskorting (credito lavoro): {ak} €\n= loonheffing: {ha} €/anno / 12 = {mens} €/mese\nAliquota effettiva: {teff} %\n\nBase giuridica: Wet IB 2001; Belastingplan {annee}.",
            "Loonheffing {annee} (trabajador por debajo de la edad AOW) — retención mensual en origen.\n\nRenta anual estimada: {rev} €\n\nEscala box 1:\n• tramo 1 (0 → {b1p} €): {b1t} % (incl. primas volksverzekeringen {pvv} %)\n• tramo 2 ({b1p} → {b2p} €): {b2t} %\n• tramo 3 (> {b2p} €): {b3t} %\n= impuesto + primas brutos: {box1} €/año\n\n− algemene heffingskorting (crédito general): {ahk} €\n− arbeidskorting (crédito laboral): {ak} €\n= loonheffing: {ha} €/año / 12 = {mens} €/mes\nTipo efectivo: {teff} %\n\nBase legal: Wet IB 2001; Belastingplan {annee}.",
        ],
        "CN_GENERIC" => [
