...
pub fn it_ivs(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        ...
        explication: format!(
            ...
            \n\n\
            Massimale contributivo {annee} : {massimale:.2} €/mois ({annuel:.2} €/an). \
            ...
            massimale = massimale,
            annuel    = massimale * dec!(12),
        ),
        loi_ref: Some("L. 335/1995 — L. 153/1969 art. 12 — Circ. INPS annuali".into()),
    }
}
...
pub fn it_naspi(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        code:        "IT_NASPI".into(),
        libelle:     "NASpI — Assicurazione chômage (cotisation ordinaire)".into(),
        base:        brut,
        ...
        explication: "La NASpI (Nuova Assicurazione Sociale per l'Impiego), instituée par \
            ...
            Aucun plafond n'est appliqué — assiette = rémunération brute totale.".into(),
        loi_ref: Some("D.Lgs. 22/2015 — L. 92/2012 — L. 228/2012".into()),
    }
}
...
pub fn it_naspi_termine(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        ...
        libelle:     "NASpI — Contributo addizionale CDD (+1,40 % pat.)".into(),
        base:        brut,
        taux_sal:    Decimal::ZERO,
        montant_sal: Decimal::ZERO,
        taux_pat:    tp,
        montant_pat: (brut * tp).round_dp(2),
        categorie:   "Disoccupazione".into(),
        explication: "Majoration patronale de 1,40 % applicable uniquement aux contrats \
            ...
            contratto di apprendistato, ni aux intermittents.".into(),
        loi_ref: Some("L. 92/2012 art. 2 c. 28-29 — D.Lgs. 81/2015".into()),
    }
}
...
pub fn it_malattia(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        ...
        libelle:     "Malattia — Indemnités journalières (INPS)".into(),
        base:        brut,
        taux_sal:    Decimal::ZERO,
        montant_sal: Decimal::ZERO,
        taux_pat:    tp,
        montant_pat: (brut * tp).round_dp(2),
        categorie:   "Previdenza sociale".into(),
        explication: "La cotisation malattia finance le versement des indemnités journalières \
            ...
            l'aliquota contributiva globale transmise à l'INPS.".into(),
        loi_ref: Some("L. 153/1969 — DPR 1026/1976 — CCNL applicable".into()),
    }
}
...
pub fn it_maternita(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        ...
        libelle:     "Maternità / Paternità — Congés parentaux (INPS)".into(),
        base:        brut,
        taux_sal:    Decimal::ZERO,
        montant_sal: Decimal::ZERO,
        taux_pat:    tp,
        montant_pat: (brut * tp).round_dp(2),
        categorie:   "Previdenza sociale".into(),
        explication: "Finance les congés parentaux obligatoires versés par l'INPS : \
            ...
            • Congé parental (congedo parentale) : jusqu'à 6 mois par parent, \
              ...
              60 % pour le 2e mois). \
            Cotisation entièrement patronale (0,46 %), stable depuis plusieurs décennies.".into(),
        loi_ref: Some("D.Lgs. 151/2001 (TU maternità) — L. 160/2019 — L. 207/2024".into()),
    }
}
...
pub fn it_fondo_garanzia(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        ...
        montant_pat: (brut * tp).round_dp(2),
        categorie:   "Previdenza sociale".into(),
        explication: "Le Fondo di Garanzia, géré par l'INPS, garantit le paiement du TFR \
            ...
            les aziende > 50 salariés depuis le 01/01/2007 (L. 296/2006 art. 1 c. 755).".into(),
        loi_ref: Some("L. 297/1982 art. 2 — L. 296/2006 art. 1 c. 755".into()),
    }
}
...
pub fn it_inail(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        ...
        explication: format!(
            ...
            \n\n\
            Le taux ({:.2} % affiché) est indicatif pour un emploi de bureau / terziario \
        ...
        loi_ref: Some("DPR 1124/1965 — DPR 1124/1965 art. 268 — DM tariffe INAIL".into()),
    }
}
...
pub fn it_tfr(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        code:        "IT_TFR".into(),
        libelle:     "TFR — Trattamento Fine Rapporto (accrual mensuel)".into(),
        base:        brut,
        taux_sal:    Decimal::ZERO,
        montant_sal: Decimal::ZERO,
        taux_pat:    tp,
        ...
        explication: format!(
            ...
            Chaque mois, l'employeur constitue une provision de {montant:.2} € \
        ...
        loi_ref: Some("L. 297/1982 — L. 296/2006 art. 1 c. 755 — DPR 917/1986 art. 17".into()),
    }
}
...
pub fn esonero_contributivo(brut: Decimal, ctx: &ContextPaie) -> Option<LigneCotisation> {
    ...
    if annee == 2022 && mois >= 7 {
        ...
        return Some(LigneCotisation {
            ...
            libelle:     "Esonero contributivo H2 2022 (−0,80 % IVS)".into(),
            base:        brut,
            taux_sal:    -taux,
            montant_sal: montant,
            ...
            explication: "Réduction temporaire de la cotisation IVS salarié de 0,80 point \
                ...
                Le montant négatif augmente le net à payer du salarié.".into(),
            loi_ref: Some("DL 115/2022 art. 20 — L. 142/2022".into()),
        });
    }
    ...
    if annee == 2023 {
        ...
        return Some(LigneCotisation {
            ...
            libelle:     format!("Esonero contributivo 2023 (−{:.0} % IVS)", taux_pp),
            base:        brut,
            taux_sal:    -taux,
            montant_sal: montant,
            taux_pat:    Decimal::ZERO,
            ...
            loi_ref: Some("L. 197/2022 art. 1 c. 281-286".into()),
        });
    }
