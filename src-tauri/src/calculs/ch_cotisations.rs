...
pub fn ch_avs(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        code:        "CH_AVS".into(),
        libelle:     ctx.libelle("CH_AVS", "AVS — Assurance-vieillesse et survivants"),
        base:        brut,
        taux_sal:    ts,
        ...
        explication: ctx.expl("CH_AVS",
            "L'Assurance-vieillesse et survivants (AVS, AHV en allemand) est le 1er pilier \
            ...
            Rente maximale 2025 : CHF 2 590/mois ; rente minimale : CHF 1 225/mois."),
        loi_ref: Some(ctx.loi_ref("Art. 5 et 13 LAVS (RS 831.10) — Réforme AVS 21 (RO 2023 92)")),
    }
}
...
pub fn ch_ai(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        code:        "CH_AI".into(),
        libelle:     ctx.libelle("CH_AI", "AI — Assurance invalidité"),
        base:        brut,
        taux_sal:    ts,
        montant_sal: (brut * ts).round_dp(2),
        taux_pat:    tp,
        montant_pat: (brut * tp).round_dp(2),
        categorie:   "1er pilier".into(),
        explication: ctx.expl("CH_AI",
            "L'Assurance invalidité (AI, IV en allemand), instituée en 1959 par la LAI, \
        ...
        loi_ref: Some(ctx.loi_ref("Art. 3 LAI (RS 831.20)")),
    }
}
...
pub fn ch_apg(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        code:        "CH_APG".into(),
        libelle:     ctx.libelle("CH_APG", "APG — Allocations pour perte de gain"),
        base:        brut,
        taux_sal:    ts,
        montant_sal: (brut * ts).round_dp(2),
        taux_pat:    tp,
        montant_pat: (brut * tp).round_dp(2),
        categorie:   "1er pilier".into(),
        explication: ctx.expl("CH_APG",
            "Les Allocations pour perte de gain (APG, EO en allemand) compensent \
            ...
            'cotisations AVS/AI/APG' = 10,60 % total (5,30 % chacun)."),
        loi_ref: Some(ctx.loi_ref("Art. 27 LAPG (RS 834.1) — Congé paternité : Loi du 25/09/2020")),
    }
}
...
pub fn ch_ac(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        code:        "CH_AC".into(),
        libelle:     ctx.libelle("CH_AC", "AC — Assurance-chômage"),
        base,
        taux_sal:    ts,
        ...
        explication: ctx.expl("CH_AC",
            ...
            (CHF {plafond} /mois). Ce plafond correspond au gain assuré maximal. \
            ...
            .replace("{plafond}", &format!("{}", CH_PLAFOND_MENSUEL)),
        loi_ref: Some(ctx.loi_ref("Art. 3 LACI (RS 837.0) — Ordonnance OACI (RS 837.02)")),
    }
}
...
pub fn ch_aanp(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        code:        "CH_AANP".into(),
        libelle:     ctx.libelle("CH_AANP", "AANP — Assurance accidents non professionnels"),
        base,
        taux_sal:    ts,
        montant_sal: (base * ts).round_dp(2),
        ...
        explication: ctx.expl("CH_AANP",
            ...
            Assiette : salaire brut plafonné à CHF 148 200/an (CHF {plafond} /mois). \
            Le taux (ici {taux} %) est fixé par l'assureur (SUVA ou assureur privé agréé) \
            selon la classe de risque professionnelle — ce taux est indicatif pour un emploi de bureau.")
            .replace("{plafond}", &format!("{}", CH_PLAFOND_MENSUEL))
            .replace("{taux}", &format!("{:.1}", ts * dec!(100))),
        loi_ref: Some(ctx.loi_ref("Art. 92 et 93 LAA (RS 832.20) — OLAA (RS 832.202)")),
    }
}
...
pub fn ch_aap(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        code:        "CH_AAP".into(),
        libelle:     ctx.libelle("CH_AAP", "AAP — Assurance accidents professionnels"),
        base,
        ...
        explication: ctx.expl("CH_AAP",
            ...
            Assiette : salaire brut plafonné à CHF 148 200/an (CHF {plafond} /mois). \
            Le taux (ici {taux} %) est déterminé par la SUVA ou un assureur agréé \
            ...
            .replace("{plafond}", &format!("{}", CH_PLAFOND_MENSUEL))
            .replace("{taux}", &format!("{:.1}", tp * dec!(100))),
        loi_ref: Some(ctx.loi_ref("Art. 92 et 93 LAA (RS 832.20) — OLAA (RS 832.202)")),
    }
}
...
pub fn ch_ijm(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        code:        "CH_IJM".into(),
        libelle:     ctx.libelle("CH_IJM", "IJM — Indemnités journalières maladie (plan collectif)"),
        base:        brut,
        taux_sal:    ts,
        montant_sal: (brut * ts).round_dp(2),
        taux_pat:    tp,
        ...
        explication: ctx.expl("CH_IJM",
            "Contrairement à la France, la Suisse n'a pas d'assurance maladie indemnités \
            ...
            selon l'assureur, le taux de sinistralité et les garanties choisies."),
        loi_ref: Some(ctx.loi_ref("Art. 67-77 LAMal (RS 832.10) — ou LCA (RS 221.229.1) selon contrat")),
    }
}
...
pub fn ch_lpp(brut: Decimal, ctx: &ContextPaie) -> LigneCotisation {
    ...
    LigneCotisation {
        code:        "CH_LPP".into(),
        libelle:     ctx.libelle("CH_LPP", "LPP — Prévoyance professionnelle (2ème pilier)"),
        base:        coord,
        taux_sal:    ts,
        ...
        explication: ctx.expl("CH_LPP",
            ...
            .replace("{coord_min}", &format!("{}", lpp_coord_min(ctx)))
            .replace("{coord_ded}", &format!("{}", lpp_deduction_coord(ctx)))
            .replace("{brut}", &format!("{}", brut))
            .replace("{coord}", &format!("{}", coord))
            .replace("{coord_max}", &format!("{}", lpp_coord_max(ctx))),
        loi_ref: Some(ctx.loi_ref("Art. 7 et 16 LPP (RS 831.40) — OPP 2 (RS 831.441.1)")),
    }
}
