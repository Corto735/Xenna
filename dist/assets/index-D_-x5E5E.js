...
`);p.forEach((g,$)=>a.set(g,v[$]??g))}l.forEach(i=>{const o=_.get(i);i.isConnected&&a.has(o)&&(i.textContent=a.get(o))}),document.documentElement.lang=e,Z=e}catch(i){console.error("Traduction échouée :",i),t.textContent="🌐 ✗",setTimeout(()=>{t.textContent="🌐 LANGUE",t.classList.remove("loading")},2e3);return}t.textContent="🌐 LANGUE",t.classList.remove("loading")};document.addEventListener("click",e=>{!e.target.closest("#trad-btn")&&!e.target.closest("#trad-panel")&&(document.getElementById("trad-panel")?.classList.remove("open"),document.getElementById("trad-btn")?.classList.remove("open"))});window.toggleA11yPanel=function(){const e=document.getElementById("a11y-panel"),t=document.getElementById("a11y-btn"),l=e.classList.toggle("open");t.classList.toggle("open",l)};window.toggleHVMode=function(){const e=document.body.classList.toggle("hv-mode");document.getElementById("hv-switch")?.classList.toggle("on",e),document.getElementById("a11y-hv-btn")?.classList.toggle("active",e),localStorage.setItem("xenna-hv",e?"1":"")};window.toggleZoom=function(){const e=document.body.classList.toggle("zoom-mode");document.documentElement.style.zoom=e?"200%":"",document.getElementById("zoom-switch")?.classList.toggle("on",e),document.getElementById("a11y-magnifier")?.classList.toggle("active",e),localStorage.setItem("xenna-zoom",e?"1":"")};const Q=new Set,Ie=new Set(["IBM Plex Mono","Fira Code","JetBrains Mono","Source Code Pro","Roboto Mono","Inconsolata"]);window.setAppFont=function(e,t=!1){if(!e){document.body.classList.remove("custom-font"),document.documentElement.style.removeProperty("--app-font"),localStorage.removeItem("xenna-font");const a=document.getElementById("font-picker");a&&(a.value="");return}if(!Ie.has(e))return;const l=e.replace(/ /g,"+");if(!Q.has(l)){const a=document.createElement("link");a.rel="stylesheet",a.href=`https://fonts.googleapis.com/css2?family=${l}&display=swap`,document.head.appendChild(a),Q.add(l)}document.documentElement.style.setProperty("--app-font",`'${e}', monospace`),document.body.classList.add("custom-font"),localStorage.setItem("xenna-font",e);const s=document.getElementById("font-picker");s&&t&&(s.value=e)};const w=[];window.scan67=function(){const e=Date.now();for(w.push(e);w.length&&e-w[0]>1500;)w.shift();const t=w.length>=3;t&&(w.length=0);const l=t?/42/:/67/,a=Array.from(document.querySelectorAll(".mob-val, .sb-val, .ascii-tbl td, .fm-val, .fm-result td")).filter(i=>l.test(i.textContent.replace(/[\s ]/g,""))&&i.offsetParent!==null);if(a.length===0)return;const n=document.getElementById("a11y-67-btn");if(n.classList.add("active"),t){const i=["#ff0055","#ff6600","#ffcc00","#00ff88","#00ccff","#aa00ff","#ff00cc","#39ff14","#ff4444","#44ffff","#ff69b4","#7fff00"];a.forEach((o,p)=>{setTimeout(()=>{const c=i[Math.floor(Math.random()*i.length)];Object.assign(o.style,{background:c,color:"#000",outline:`2px solid ${c}`,borderRadius:"2px",transition:"all 0.15s"}),setTimeout(()=>Object.assign(o.style,{background:"",color:"",outline:"",borderRadius:""}),900)},p*250)}),setTimeout(()=>n.classList.remove("active"),a.length*250+1e3)}else a.forEach((i,o)=>{setTimeout(()=>{i.classList.remove("flash-67"),i.offsetWidth,i.classList.add("flash-67"),i.addEventListener("animationend",()=>i.classList.remove("flash-67"),{once:!0})},o*500)}),setTimeout(()=>n.classList.remove("active"),a.length*500+200)};window.toggleBWMode=function(){const e=document.body.classList.toggle("bw-mode");document.getElementById("bw-switch")?.classList.toggle("on",e),document.getElementById("a11y-bw-btn")?.classList.toggle("active",e),localStorage.setItem("xenna-bw",e?"1":"")};function m(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}window.setView=function(e){["mobile","desktop","annuel","forge","apropos","gaabrielle","hercule"].forEach(t=>document.body.classList.toggle("is-"+t,e===t)),document.getElementById("btn-desk").classList.toggle("active",e==="desktop"),document.getElementById("btn-mob").classList.toggle("active",e==="mobile"),document.getElementById("btn-ann").classList.toggle("active",e==="annuel"),D&&(e==="desktop"||e==="mobile")&&re(D),e==="forge"&&Te()};let A="EUR";function d(e){const t=parseFloat(e),l=A==="CHF"?" CHF":" €";return t.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+l}function Se(e,t=!1){const l=parseFloat(e),s=A==="CHF"?" CHF":" €",a=l.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+s;return t&&l>0?"+"+a:a}function B(e){return(parseFloat(e)*100).toFixed(2)+" %"}function ce(){const e=document.body.classList.contains("is-mobile")?"m-date":"d-date",t=e==="m-date"?"d-date":"m-date";return document.getElementById(e)?.value||document.getElementById(t)?.value||oe}function j(e){if(!e)return"—";const[t,l,s]=e.split("-");return`${s}/${l}/${t}`}const we=[{min:0,max:1620,taux:0},{min:1620,max:1683,taux:.005},{min:1683,max:1791,taux:.013},{min:1791,max:1911,taux:.021},{min:1911,max:2042,taux:.029},{min:2042,max:2151,taux:.035},{min:2151,max:2294,taux:.041},{min:2294,max:2714,taux:.053},{min:2714,max:3107,taux:.075},{min:3107,max:3539,taux:.099},{min:3539,max:3983,taux:.119},{min:3983,max:4648,taux:.138},{min:4648,max:5574,taux:.158},{min:5574,max:6974,taux:.179},{min:6974,max:8711,taux:.2},{min:8711,max:12091,taux:.24},{min:12091,max:16376,taux:.28},{min:16376,max:25706,taux:.33},{min:25706,max:55062,taux:.38},{min:55062,max:1/0,taux:.43}];function z(e){const t=parseFloat(e);if(isNaN(t)||t<=0)return{total:0,taux_effectif:0,details:[]};let l=0;const s=[];for(const a of we){if(t<=a.min)break;const i=+((a.max===1/0?t:Math.min(t,a.max))-a.min).toFixed(2),o=i*a.taux;if(s.push({min:a.min,max:a.max===1/0?null:a.max,taux:a.taux,base:i,montant:+o.toFixed(2)}),l+=o,a.max===1/0||t<=a.max)break}return{total:+l.toFixed(2),taux_effectif:t>0?l/t:0,details:s}}const ee={"Sécurité Sociale":"cat-ss","CSG/CRDS":"cat-csg","Retraite complémentaire":"cat-ret",Prévoyance:"cat-prev",Chômage:"cat-cho",Allègement:"cat-alleg","1er pilier":"cat-ss","Assurance chômage":"cat-cho","Assurance accidents":"cat-acc","Prévoyance maladie":"cat-prev","Prévoyance (LPP)":"cat-ret","Assurance pension":"cat-ret","Assurance maladie":"cat-ss","Assurance dépendance":"cat-prev","Mutualité des employeurs":"cat-ss"},F={},te={SS_VIEILLESSE_PLAF:"min(Salaire brut, Plafond Mensuel Sécurité Sociale — PMSS)",CHOMAGE:"min(Salaire brut, 4 × PMSS)",AGS:"min(Salaire brut, 4 × PMSS)",CSG_DEDUCTIBLE:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels (CSS art. L136-2)",CSG_NON_DEDUCTIBLE:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels",CRDS:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels",AGIRC_ARRCO_T1:"min(Salaire brut, PMSS)  — Tranche 1 (entre 0 et 1 PMSS)",AGIRC_ARRCO_CEG_T1:"min(Salaire brut, PMSS)  — Tranche 1",PREVOYANCE_CADRE_MIN:"min(Salaire brut, PMSS)  — Tranche A",AGIRC_ARRCO_T2:"Fraction du salaire entre 1 PMSS et 8 PMSS  — Tranche 2",AGIRC_ARRCO_CEG_T2:"Fraction du salaire entre 1 PMSS et 8 PMSS  — Tranche 2"};function H(e,t=!0){const l=["f","(","x",")"].map((s,a)=>`<span style="animation-delay:${a*45}ms">${s}</span>`).join("");return t?`<span class="formula-star" data-fmkey="${e}" onclick="event.stopPropagation();showFormula('${e}')">${l}</span>`:`<span class="formula-star" aria-hidden="true">${l}</span>`}window.togglePasDetail=function(e){const t=document.getElementById(e);if(!t)return;const l=t.style.display!=="none";t.style.display=l?"none":"block";const s=document.getElementById(e+"-arrow");s&&(s.textContent=l?"▶":"▼")};function N(e,t){if(e.code==="REDUCTION_FILLON")return`<pre class="fm-fillon">${m(e.explication)}</pre>`;const l=t==="sal",s=l?e.taux_sal:e.taux_pat,a=parseFloat(e.base),n=l?parseFloat(e.montant_sal):Math.abs(parseFloat(e.montant_pat)),i=l?"Taux salarial":"Taux patronal",o=l?"Montant salarial":t==="alleg"?"Montant allègement":"Montant patronal",p=l?"c-sal":t==="alleg"?"c-alleg":"c-pat",c=te[e.code]?`<div class="fm-base-note">Assiette  =  ${m(te[e.code])}</div>`:"";return`
    ...
    <div class="summary-bar">
      ...
      <div class="sb-cell">
        ...
        <div class="sb-ded">
          ...
          ${s?"":`<div class="sb-ded-row" style="cursor:pointer" onclick="togglePasDetail('pas-detail-desk')">
            ...
            <span style="color:var(--purple)">− ${d(i.total)}</span>
          </div>
      ...
      <div class="sb-cell">
    ...
    </div>`,c=l.filter(u=>u.categorie!=="Allègement"&&(parseFloat(u.montant_sal)>0||u.taux_sal!=="0"||parseFloat(u.montant_pat)>0)),r=l.filter(u=>u.categorie==="Allègement"),f=c.reduce((u,E)=>u+parseFloat(E.montant_pat),0);function h(u,E){return u.map((y,x)=>{const I=E+x,S=ee[y.categorie]||"cat-ss",q=parseFloat(y.montant_sal)>0?"c-sal":"c-dim",k=parseFloat(y.montant_pat)>0?"c-pat":"c-dim",T=`${y.code}_sal`,J=`${y.code}_pat`,X=parseFloat(y.montant_sal)>0,W=parseFloat(y.montant_pat)>0;X&&(F[T]={c:y,type:"sal"}),W&&(F[J]={c:y,type:"pat"});const ve=H(T,X),ye=H(J,W);return`
        <tr class="data-row" id="row-${I}" onclick="toggleExpl(${I})">
          ...
          <td class="r">${B(y.taux_sal)}</td>
          <td class="r ${q}">${d(y.montant_sal)}${ve}</td>
          <td class="r">${B(y.taux_pat)}</td>
          <td class="r ${k}">${d(y.montant_pat)}${ye}</td>
        </tr>
        <tr class="expl-row" id="expl-${I}" style="display:none">
          <td colspan="6">
            <div class="expl-box">
              <div class="expl-txt">▸ ${m(y.explication)}</div>
              ${y.loi_ref?`<div class="expl-ref">§ ${m(y.loi_ref)}</div>`:""}
            </div>
          </td>
        </tr>`}).join("")}const v=`
    <colgroup>
  ...
  </div>`,C=r.reduce((u,E)=>u+parseFloat(E.montant_pat),0),b=r.length===0?"":`
    <div class="tbl-section-head">── ALLÈGEMENTS PATRONAUX ───────────────────────────────────────────</div>
    <table class="ascii-tbl">
      ${v}
      <tbody>
        ${r.map((u,E)=>{const y=c.length+E,x=ee[u.categorie]||"cat-alleg",I=Math.abs(parseFloat(u.montant_pat)),S=`${u.code}_alleg`;return F[S]={c:u,type:"alleg"},`
            <tr class="data-row" id="row-${y}" onclick="toggleExpl(${y})">
              ...
              <td class="r"></td>
              <td class="r"></td>
              <td class="r c-alleg">${B(Math.abs(parseFloat(u.taux_pat)))}</td>
              <td class="r c-alleg">− ${d(I)}${H(S)}</td>
            </tr>
        ...
        <tr class="tbl-total">
          ...
          <td class="r c-alleg">− ${d(Math.abs(C))}</td>
        </tr>
      </tbody>
    ...
    <div class="${`mob-stripe-${a}-${n%2===0?"a":"b"}`}">
      ...
      ${b.loi_ref?`<div class="mob-exp-loi">§ ${m(b.loi_ref)}</div>`:""}`,q=`mob-stripe-sal-${u%2===0?"a":"b"}`,k=E?`<span class="mob-val mob-cot-amt" style="color:#ffe033" onclick="mobToggle('${x}','how')">− ${d(b.montant_sal)}</span>`:`<span class="mob-val c-dim">0 ${A==="CHF"?"CHF":"€"}</span>`,T=y?`<span class="mob-val c-orange mob-cot-amt" onclick="mobToggle('${x}','how')">+ ${d(b.montant_pat)}</span>`:`<span class="mob-val c-dim">0 ${A==="CHF"?"CHF":"€"}</span>`;return`
      <div class="${q}">
        <div class="mob-row">
          <span class="mob-lbl mob-cot-lbl" onclick="mobToggle('${x}','why')">${m(b.libelle)}</span>
          <span style="display:flex;flex-direction:column;align-items:flex-end;gap:0.1rem">${k}${T}</span>
        </div>
      ...
      </div>`}).join(""),C=h.map((b,u)=>Ce(b,`${b.code}_alleg`,`− ${d(Math.abs(parseFloat(b.montant_pat)))}`,"c-alleg","alleg",u)).join("");t.innerHTML=`
    ...
    </div>`}function re(e){A=e.devise||"EUR",Le(e),_e(e)}function le(e){const t=`<div style="padding:1.5rem;color:#f87171;font-size:0.8rem">⚠ ${m(e)}</div>`;document.getElementById("res-desktop").innerHTML=t,document.getElementById("res-mobile").innerHTML=t}async function de(e){const t=e==="mobile",l=document.getElementById(t?"m-brut":"d-brut").value,s=document.getElementById(t?"m-statut":"d-statut").value,a=document.getElementById(t?"m-nom":"d-nom").value||"Dupont",n=document.getElementById(t?"m-prenom":"d-prenom").value||"Marie",i=document.getElementById(t?"m-date":"d-date").value||oe,o=document.getElementById(t?"m-alsace-moselle":"d-alsace-moselle")?.checked??!1,p=document.getElementById(t?"m-suisse":"d-suisse")?.checked??!1,c=document.getElementById(t?"m-luxembourg":"d-luxembourg")?.checked??!1,r=parseFloat(l);if(!l||isNaN(r)||r<=0){le("Salaire brut invalide — saisir un montant positif.");return}if(!/^\d{4}-\d{2}-\d{2}$/.test(i)){le(`Date invalide : '${i}' (format attendu : YYYY-MM-DD).`);return}["d-brut","m-brut"].forEach(v=>{const g=document.getElementById(v);g&&(g.value=l)}),["d-statut","m-statut"].forEach(v=>{const g=document.getElementById(v);g&&(g.value=s)}),["d-nom","m-nom"].forEach(v=>{const g=document.getElementById(v);g&&(g.value=a)}),["d-prenom","m-prenom"].forEach(v=>{const g=document.getElementById(v);g&&(g.value=n)}),["d-date","m-date"].forEach(v=>{const g=document.getElementById(v);g&&(g.value=i)});const f=p?"suisse":c?"luxembourg":null,h=f?"2026-01-01":i;try{const v=await ne("calculer_bulletin",{salarie:{nom:a,prenom:n,salaire_brut:l.toString(),statut:s,alsace_moselle:o,pays:f??"france"},datePaie:h});D=v,re(v)}catch(v){console.error("[calculer_bulletin] erreur brute :",v);const g=P(v),$=`<div style="padding:1.5rem;color:#f87171;font-size:0.8rem">ERREUR : ${m(g)}</div>`;document.getElementById("res-desktop").innerHTML=$,document.getElementById("res-mobile").innerHTML=$}}function Me(e){const t=document.getElementById("res-annuel"),l=e.lignes,s=l.map(r=>r.smic),a=`
    <thead><tr>
      ...
      <th>NET</th>
      <th>COÛT EMPL.</th>
    </tr></thead>`,n=l.map((r,f)=>{const h=f>0&&r.smic!==s[f-1],v=r.mois_libelle.includes("13e"),g=parseFloat(r.fillon_regularise)-parseFloat(r.fillon_simple),$=Math.abs(g)<.005?'<span style="color:var(--dim)">—</span>':`<span class="delta-nonzero">${g>0?"+":""}${Se(g.toFixed(2))}</span>`;return`<tr class="${[h?"smic-change":"",v?"treizieme-mois":""].filter(Boolean).join(" ")}">
    ...
    </div>`}async function Fe(e){forgeNav("detail");const t=document.getElementById("forge-profil-content");t.innerHTML='<div style="color:var(--muted);font-size:0.75rem">chargement…</div>';try{let l=L.find(s=>s.pseudo.toLowerCase()===e.toLowerCase());if(!l){const s=await fetch(`/profil/${encodeURIComponent(e)}`);if(!s.ok)throw new Error(`HTTP ${s.status} — ${await s.text()||s.statusText}`);l=await s.json()}t.innerHTML=Ne(l)}catch(l){t.innerHTML=`<div style="color:var(--red);font-size:0.75rem">Erreur : ${m(P(l))}</div>`}}function Ne(e){const t=e.linkedin_url?`<a class="profil-linkedin" href="${m(e.linkedin_url)}" target="_blank" rel="noopener noreferrer">↗ LinkedIn</a>`:"",s=[{niveau:"Maîtrisée",cls:"m",items:e.expertises.filter(i=>i.niveau==="Maîtrisée")},{niveau:"Pratiquée",cls:"p",items:e.expertises.filter(i=>i.niveau==="Pratiquée")},{niveau:"Connue",cls:"c",items:e.expertises.filter(i=>i.niveau==="Connue")}].filter(i=>i.items.length>0).map(i=>`
