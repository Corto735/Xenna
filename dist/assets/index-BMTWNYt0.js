(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function l(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(a){if(a.ep)return;a.ep=!0;const n=l(a);fetch(a.href,n)}})();const _e="modulepreload",we=function(e){return"/"+e},te={},Le=function(t,l,s){let a=Promise.resolve();if(l&&l.length>0){let i=function(f){return Promise.all(f.map(p=>Promise.resolve(p).then(c=>({status:"fulfilled",value:c}),c=>({status:"rejected",reason:c}))))};document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),r=o?.nonce||o?.getAttribute("nonce");a=i(l.map(f=>{if(f=we(f),f in te)return;te[f]=!0;const p=f.endsWith(".css"),c=p?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${f}"]${c}`))return;const g=document.createElement("link");if(g.rel=p?"stylesheet":_e,p||(g.as="script"),g.crossOrigin="",g.href=f,r&&g.setAttribute("nonce",r),document.head.appendChild(g),p)return new Promise((y,h)=>{g.addEventListener("load",y),g.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${f}`)))})}))}function n(i){const o=new Event("vite:preloadError",{cancelable:!0});if(o.payload=i,window.dispatchEvent(o),!o.defaultPrevented)throw i}return a.then(i=>{for(const o of i||[])o.status==="rejected"&&n(o.reason);return t().catch(n)})};async function me(e,t={}){if(window.__TAURI_INTERNALS__){const{invoke:s}=await Le(async()=>{const{invoke:a}=await import("./core-DV6XEvTN.js");return{invoke:a}},[]);return s(e,t)}const l=await fetch(`/api/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!l.ok)throw await l.text();return l.json()}function D(e){if(e==null)return"(erreur nulle — redémarre l'app ou ouvre DevTools Ctrl+Shift+I)";if(typeof e=="string")return e||"(erreur muette — ouvre DevTools Ctrl+Shift+I et consulte la Console)";if(e instanceof Error)return e.message||e.toString();try{return JSON.stringify(e,null,2)}catch{return String(e)}}let X=null;const W="2026-01-31",pe=W;document.addEventListener("DOMContentLoaded",()=>{["d-date","m-date"].forEach(n=>{const i=document.getElementById(n);i&&(i.value=W,i.max=W)}),document.addEventListener("keydown",n=>{n.key==="Escape"&&closeFmModal()}),window._heroH=Math.random()<.015?{prenom:"Jean-Noël",nom:"Favari"}:ue(Ge),window._heroF=ue(Ve),xe(window._heroH.prenom,window._heroH.nom),Ie("H"),["d-prenom","m-prenom","d-nom","m-nom"].forEach(n=>{document.getElementById(n)?.addEventListener("input",()=>{Q=!0})});const e=Math.round(O*100),t=Math.round(O/(1-O)*100);document.querySelectorAll(".genre-ecart-hint").forEach(n=>{n.dataset.textFh=`// −${e} % · écart salarial F/H`,n.dataset.textHf=`// +${t} % · écart salarial H/F`});const l=window.matchMedia("(max-width: 680px)"),s=n=>{const i=document.body;!i.classList.contains("is-annuel")&&!i.classList.contains("is-forge")&&!i.classList.contains("is-apropos")&&!i.classList.contains("is-gaabrielle")&&!i.classList.contains("is-hercule")&&setView(n.matches?"mobile":"desktop")};l.addEventListener("change",s),s(l),localStorage.getItem("xenna-hv")&&(document.body.classList.add("hv-mode"),document.getElementById("hv-switch")?.classList.add("on")),localStorage.getItem("xenna-zoom")&&(document.body.classList.add("zoom-mode"),document.documentElement.style.zoom="200%",document.getElementById("zoom-switch")?.classList.add("on"),document.getElementById("a11y-magnifier")?.classList.add("active"));const a=localStorage.getItem("xenna-font");a&&setAppFont(a,!0),localStorage.getItem("xenna-hv")&&document.getElementById("a11y-hv-btn")?.classList.add("active"),localStorage.getItem("xenna-bw")&&(document.body.classList.add("bw-mode"),document.getElementById("bw-switch")?.classList.add("on"),document.getElementById("a11y-bw-btn")?.classList.add("active")),localStorage.getItem("xenna-dactylo")&&(A=!0,document.getElementById("dactylo-switch")?.classList.add("on")),document.addEventListener("click",n=>{!n.target.closest("#a11y-btn")&&!n.target.closest("#a11y-panel")&&(document.getElementById("a11y-panel")?.classList.remove("open"),document.getElementById("a11y-btn")?.classList.remove("open"))})});let le="fr";const J={},R=new Map;function Me(){const e="script,style,input,select,textarea,.mob-val,.sb-val,.fm-val,.a11y-float,.trad-panel,#a11y-panel",t=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT,{acceptNode(s){const a=s.textContent.trim();return!a||a.length<2||/^[\d\s,.\-+%€×\/:()[\]]+$/.test(a)||s.parentElement?.closest(e)?NodeFilter.FILTER_REJECT:NodeFilter.FILTER_ACCEPT}}),l=[];for(;t.nextNode();)l.push(t.currentNode);return l}window.toggleTradPanel=function(){const e=document.getElementById("trad-panel"),t=document.getElementById("trad-btn"),l=e.classList.toggle("open");t.classList.toggle("open",l)};window.translateApp=async function(e){if(document.getElementById("trad-panel")?.classList.remove("open"),document.getElementById("trad-btn")?.classList.remove("open"),document.querySelectorAll(".trad-lang-btn").forEach(i=>i.classList.remove("active")),document.querySelector(`.trad-lang-btn[onclick="translateApp('${e}')"]`)?.classList.add("active"),e==="fr"){R.forEach((i,o)=>{o.isConnected&&(o.textContent=i)}),document.documentElement.lang="fr",le="fr";return}const t=document.getElementById("trad-btn");t.classList.add("loading"),t.textContent="🌐 …";const l=Me();l.forEach(i=>{R.has(i)||R.set(i,i.textContent)});const s=l.map(i=>R.get(i));J[e]||(J[e]=new Map);const a=J[e],n=[...new Set(s)].filter(i=>!a.has(i));try{if(n.length>0)for(let o=0;o<n.length;o+=20){const r=n.slice(o,o+20),f=r.join(`

`),p=`https://api.mymemory.translated.net/get?q=${encodeURIComponent(f)}&langpair=fr|${e}`,c=await fetch(p);if(!c.ok)throw new Error("HTTP "+c.status);const y=(await c.json()).responseData.translatedText.split(`

`);r.forEach((h,I)=>a.set(h,y[I]??h))}l.forEach(i=>{const o=R.get(i);i.isConnected&&a.has(o)&&(i.textContent=a.get(o))}),document.documentElement.lang=e,le=e}catch(i){console.error("Traduction échouée :",i),t.textContent="🌐 ✗",setTimeout(()=>{t.textContent="🌐 LANGUE",t.classList.remove("loading")},2e3);return}t.textContent="🌐 LANGUE",t.classList.remove("loading")};document.addEventListener("click",e=>{!e.target.closest("#trad-btn")&&!e.target.closest("#trad-panel")&&(document.getElementById("trad-panel")?.classList.remove("open"),document.getElementById("trad-btn")?.classList.remove("open"))});window.toggleA11yPanel=function(){const e=document.getElementById("a11y-panel"),t=document.getElementById("a11y-btn"),l=e.classList.toggle("open");t.classList.toggle("open",l)};window.toggleHVMode=function(){const e=document.body.classList.toggle("hv-mode");document.getElementById("hv-switch")?.classList.toggle("on",e),document.getElementById("a11y-hv-btn")?.classList.toggle("active",e),localStorage.setItem("xenna-hv",e?"1":"")};window.toggleZoom=function(){const e=document.body.classList.toggle("zoom-mode");document.documentElement.style.zoom=e?"200%":"",document.getElementById("zoom-switch")?.classList.toggle("on",e),document.getElementById("a11y-magnifier")?.classList.toggle("active",e),localStorage.setItem("xenna-zoom",e?"1":"")};const se=new Set,Pe=new Set(["IBM Plex Mono","Fira Code","JetBrains Mono","Source Code Pro","Roboto Mono","Inconsolata"]);window.setAppFont=function(e,t=!1){if(!e){document.body.classList.remove("custom-font"),document.documentElement.style.removeProperty("--app-font"),localStorage.removeItem("xenna-font");const a=document.getElementById("font-picker");a&&(a.value="");return}if(!Pe.has(e))return;const l=e.replace(/ /g,"+");if(!se.has(l)){const a=document.createElement("link");a.rel="stylesheet",a.href=`https://fonts.googleapis.com/css2?family=${l}&display=swap`,document.head.appendChild(a),se.add(l)}document.documentElement.style.setProperty("--app-font",`'${e}', monospace`),document.body.classList.add("custom-font"),localStorage.setItem("xenna-font",e);const s=document.getElementById("font-picker");s&&t&&(s.value=e)};const M=[];window.scan67=function(){const e=Date.now();for(M.push(e);M.length&&e-M[0]>1500;)M.shift();const t=M.length>=3;t&&(M.length=0);const l=t?/42/:/67/,a=Array.from(document.querySelectorAll(".mob-val, .sb-val, .ascii-tbl td, .fm-val, .fm-result td")).filter(i=>l.test(i.textContent.replace(/[\s ]/g,""))&&i.offsetParent!==null);if(a.length===0)return;const n=document.getElementById("a11y-67-btn");if(n.classList.add("active"),t){const i=["#ff0055","#ff6600","#ffcc00","#00ff88","#00ccff","#aa00ff","#ff00cc","#39ff14","#ff4444","#44ffff","#ff69b4","#7fff00"];a.forEach((o,r)=>{setTimeout(()=>{const f=i[Math.floor(Math.random()*i.length)];Object.assign(o.style,{background:f,color:"#000",outline:`2px solid ${f}`,borderRadius:"2px",transition:"all 0.15s"}),setTimeout(()=>Object.assign(o.style,{background:"",color:"",outline:"",borderRadius:""}),900)},r*250)}),setTimeout(()=>n.classList.remove("active"),a.length*250+1e3)}else a.forEach((i,o)=>{setTimeout(()=>{i.classList.remove("flash-67"),i.offsetWidth,i.classList.add("flash-67"),i.addEventListener("animationend",()=>i.classList.remove("flash-67"),{once:!0})},o*500)}),setTimeout(()=>n.classList.remove("active"),a.length*500+200)};window.toggleBWMode=function(){const e=document.body.classList.toggle("bw-mode");document.getElementById("bw-switch")?.classList.toggle("on",e),document.getElementById("a11y-bw-btn")?.classList.toggle("active",e),localStorage.setItem("xenna-bw",e?"1":"")};let A=!1,ie=0;const ae=e=>new Promise(t=>setTimeout(t,e));window.toggleDactylo=function(){A=!A,document.getElementById("dactylo-switch")?.classList.toggle("on",A),localStorage.setItem("xenna-dactylo",A?"1":"")};async function Ae(){const e=++ie,t=()=>e!==ie,l=document.querySelectorAll("#res-desktop tr.data-row");if(!l.length)return;const s=[];for(const a of l){const n=a.querySelector("td:first-child > span:last-child");if(n){const i=n.textContent;n.textContent="",s.push({target:n,text:i,ms:4})}a.querySelectorAll("td:not(:first-child)").forEach(i=>{const o=[...i.childNodes].find(r=>r.nodeType===3&&r.textContent.trim());if(o){const r=o.textContent;o.textContent="",s.push({target:o,text:r,ms:2})}}),s.push({pause:8})}for(const a of s){if(t())return;if(a.pause){await ae(a.pause);continue}for(const n of a.text){if(t())return;a.target.textContent+=n,await ae(a.ms)}}}function m(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}window.setView=function(e){["mobile","desktop","annuel","forge","apropos","contact","gaabrielle","hercule"].forEach(t=>document.body.classList.toggle("is-"+t,e===t)),document.getElementById("btn-desk").classList.toggle("active",e==="desktop"),document.getElementById("btn-mob").classList.toggle("active",e==="mobile"),document.getElementById("btn-ann").classList.toggle("active",e==="annuel"),X&&(e==="desktop"||e==="mobile")&&ge(X),e==="forge"&&He()};let H="EUR";function d(e){const t=parseFloat(e),l=H==="CHF"?" CHF":" €";return t.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+l}function Te(e,t=!1){const l=parseFloat(e),s=H==="CHF"?" CHF":" €",a=l.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+s;return t&&l>0?"+"+a:a}function z(e){return(parseFloat(e)*100).toFixed(2)+" %"}function be(){const e=document.body.classList.contains("is-mobile")?"m-date":"d-date",t=e==="m-date"?"d-date":"m-date";return document.getElementById(e)?.value||document.getElementById(t)?.value||pe}function K(e){if(!e)return"—";const[t,l,s]=e.split("-");return`${s}/${l}/${t}`}const Be=[{min:0,max:1620,taux:0},{min:1620,max:1683,taux:.005},{min:1683,max:1791,taux:.013},{min:1791,max:1911,taux:.021},{min:1911,max:2042,taux:.029},{min:2042,max:2151,taux:.035},{min:2151,max:2294,taux:.041},{min:2294,max:2714,taux:.053},{min:2714,max:3107,taux:.075},{min:3107,max:3539,taux:.099},{min:3539,max:3983,taux:.119},{min:3983,max:4648,taux:.138},{min:4648,max:5574,taux:.158},{min:5574,max:6974,taux:.179},{min:6974,max:8711,taux:.2},{min:8711,max:12091,taux:.24},{min:12091,max:16376,taux:.28},{min:16376,max:25706,taux:.33},{min:25706,max:55062,taux:.38},{min:55062,max:1/0,taux:.43}];function Z(e){const t=parseFloat(e);if(isNaN(t)||t<=0)return{total:0,taux_effectif:0,details:[]};let l=0;const s=[];for(const a of Be){if(t<=a.min)break;const i=+((a.max===1/0?t:Math.min(t,a.max))-a.min).toFixed(2),o=i*a.taux;if(s.push({min:a.min,max:a.max===1/0?null:a.max,taux:a.taux,base:i,montant:+o.toFixed(2)}),l+=o,a.max===1/0||t<=a.max)break}return{total:+l.toFixed(2),taux_effectif:t>0?l/t:0,details:s}}const ne={"Sécurité Sociale":"cat-ss","CSG/CRDS":"cat-csg","Retraite complémentaire":"cat-ret",Prévoyance:"cat-prev",Chômage:"cat-cho",Allègement:"cat-alleg","1er pilier":"cat-ss","Assurance chômage":"cat-cho","Assurance accidents":"cat-acc","Prévoyance maladie":"cat-prev","Prévoyance (LPP)":"cat-ret","Assurance pension":"cat-ret","Assurance maladie":"cat-ss","Assurance dépendance":"cat-prev","Mutualité des employeurs":"cat-ss"},P={},oe={SS_VIEILLESSE_PLAF:"min(Salaire brut, Plafond Mensuel Sécurité Sociale — PMSS)",CHOMAGE:"min(Salaire brut, 4 × PMSS)",AGS:"min(Salaire brut, 4 × PMSS)",CSG_DEDUCTIBLE:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels (CSS art. L136-2)",CSG_NON_DEDUCTIBLE:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels",CRDS:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels",AGIRC_ARRCO_T1:"min(Salaire brut, PMSS)  — Tranche 1 (entre 0 et 1 PMSS)",AGIRC_ARRCO_CEG_T1:"min(Salaire brut, PMSS)  — Tranche 1",PREVOYANCE_CADRE_MIN:"min(Salaire brut, PMSS)  — Tranche A",AGIRC_ARRCO_T2:"Fraction du salaire entre 1 PMSS et 8 PMSS  — Tranche 2",AGIRC_ARRCO_CEG_T2:"Fraction du salaire entre 1 PMSS et 8 PMSS  — Tranche 2"};function q(e,t=!0){const l=["f","(","x",")"].map((s,a)=>`<span style="animation-delay:${a*45}ms">${s}</span>`).join("");return t?`<span class="formula-star" data-fmkey="${e}" onclick="event.stopPropagation();showFormula('${e}')">${l}</span>`:`<span class="formula-star" aria-hidden="true">${l}</span>`}window.togglePasDetail=function(e){const t=document.getElementById(e);if(!t)return;const l=t.style.display!=="none";t.style.display=l?"none":"block";const s=document.getElementById(e+"-arrow");s&&(s.textContent=l?"▶":"▼")};function k(e,t){if(e.code==="REDUCTION_FILLON")return`<pre class="fm-fillon">${m(e.explication)}</pre>`;const l=t==="sal",s=l?e.taux_sal:e.taux_pat,a=parseFloat(e.base),n=l?parseFloat(e.montant_sal):Math.abs(parseFloat(e.montant_pat)),i=l?"Taux salarial":"Taux patronal",o=l?"Montant salarial":t==="alleg"?"Montant allègement":"Montant patronal",r=l?"c-sal":t==="alleg"?"c-alleg":"c-pat",f=oe[e.code]?`<div class="fm-base-note">Assiette  =  ${m(oe[e.code])}</div>`:"";return`
    ...
    <table class="fm-calc">
      ...
      <tr class="fm-result fm-sep">
        <td>${o}</td>
        <td class="fm-op">=</td>
        <td class="fm-val ${r}">${d(n)}</td>
      </tr>
    </table>`}function fe(e){const t=Z(e);return`
    ...
    </table>`}window.showFormula=function(e){const t=P[e];if(!t)return;const l=document.getElementById("fm-body");if(t.type==="pas"){document.getElementById("fm-title").textContent="Prélèvement à la Source (PAS)",document.getElementById("fm-badge").textContent="── Détail par tranche — barème neutre mensuel DGFIP ─────────",l.className="fm-type-pas",l.innerHTML=fe(t.netImposable),document.getElementById("fm-modal").classList.add("open"),document.querySelectorAll(`[data-fmkey="${e}"]`).forEach(o=>o.classList.add("visited"));return}const{c:s,type:a}=t,n=a==="sal",i=s.code==="REDUCTION_FILLON"?"── Allègement patronal ──────────────────────":n?"── Part salariale ───────────────────────────":"── Part patronale ───────────────────────────";document.getElementById("fm-title").textContent=s.libelle,document.getElementById("fm-badge").textContent=i,l.className=`fm-type-${a}`,l.innerHTML=k(s,a),document.getElementById("fm-modal").classList.add("open"),document.querySelectorAll(`[data-fmkey="${e}"]`).forEach(o=>o.classList.add("visited"))};window.closeFmModal=function(){document.getElementById("fm-modal").classList.remove("open")};window.toggleExpl=function(e){const t=document.getElementById(`row-${e}`),l=document.getElementById(`expl-${e}`);if(!t||!l)return;const s=l.style.display!=="none";l.style.display=s?"none":"table-row",t.classList.toggle("open",!s)};function Fe(e){const t=document.getElementById("res-desktop"),l=e.cotisations,s=["suisse","luxembourg"].includes(e.salarie?.pays),a=l.reduce((u,x)=>u+parseFloat(x.montant_sal),0),n=l.reduce((u,x)=>u+parseFloat(x.montant_pat),0),i=s?{total:0,taux_effectif:0}:Z(e.net_imposable),o=parseFloat(e.net_a_payer)-i.total;s||(P.PAS={type:"pas",netImposable:parseFloat(e.net_imposable)});const r=e.salarie?.pays==="suisse"?l.find(u=>u.code==="CH_IS"):null,f=r?parseFloat(r.montant_sal):0,p=r?parseFloat(r.taux_sal):0;r&&(P.CH_IS={c:r,type:"sal"});const c=a-f,g=`
      ...
      <div class="sb-cell">
        ...
        <div class="sb-ded">
          <div class="sb-ded-row">
            ...
            <span style="color:var(--red)">− ${d(c)}</span>
          </div>
          ${r?`<div class="sb-ded-row">
            <span>Impôt à la source (${(p*100).toFixed(1)} %)</span>
            <span class="fm-val" style="color:var(--purple);cursor:pointer" onclick="showFormula('CH_IS')">− ${d(f)}${q("CH_IS")}</span>
          </div>`:""}
          ${s?"":`<div class="sb-ded-row">
            <span>PAS (${(i.taux_effectif*100).toFixed(1)} %)</span>
            <span class="fm-val" style="color:var(--purple);cursor:pointer" onclick="showFormula('PAS')">− ${d(i.total)}${q("PAS")}</span>
          </div>`}
    ...
    </div>`,y=l.filter(u=>u.categorie!=="Allègement"&&(parseFloat(u.montant_sal)>0||u.taux_sal!=="0"||parseFloat(u.montant_pat)>0)),h=l.filter(u=>u.categorie==="Allègement"),I=y.reduce((u,x)=>u+parseFloat(x.montant_pat),0);function w(u,x){return u.map((v,S)=>{const C=x+S,_=ne[v.categorie]||"cat-ss",j=parseFloat(v.montant_sal)>0?"c-sal":"c-dim",V=parseFloat(v.montant_pat)>0?"c-pat":"c-dim",B=`${v.code}_sal`,F=`${v.code}_pat`,N=parseFloat(v.montant_sal)>0,Y=parseFloat(v.montant_pat)>0;N&&(P[B]={c:v,type:"sal"}),Y&&(P[F]={c:v,type:"pat"});const Se=q(B,N),Ce=q(F,Y);return`
        <tr class="data-row" id="row-${C}" onclick="toggleExpl(${C})">
          <td>
            <span class="expand-icon">▶</span>
            <span class="cat ${_}">[${v.categorie}]</span>
            <span>${v.libelle}</span>
          </td>
          <td class="r">${d(v.base)}</td>
          <td class="r">${z(v.taux_sal)}</td>
          <td class="r ${j}"${N?` onclick="event.stopPropagation();showFormula('${B}')" style="cursor:pointer"`:""}>${d(v.montant_sal)}${Se}</td>
          <td class="r">${z(v.taux_pat)}</td>
          <td class="r ${V}"${Y?` onclick="event.stopPropagation();showFormula('${F}')" style="cursor:pointer"`:""}>${d(v.montant_pat)}${Ce}</td>
        </tr>
        <tr class="expl-row" id="expl-${C}" style="display:none">
          <td colspan="6">
            <div class="expl-box">
    ...
    </table>`,L=`<div class="sim-period">
    SIMULATION AU <span class="sp-accent">${K(be())}</span>
    &nbsp;·&nbsp; PMSS en vigueur calculé depuis la base de données sans le moindre état d'âme
  </div>`,U=h.reduce((u,x)=>u+parseFloat(x.montant_pat),0),b=h.length===0?"":`
    <div class="tbl-section-head">── ALLÈGEMENTS PATRONAUX ───────────────────────────────────────────</div>
    <table class="ascii-tbl">
      ${E}
      <tbody>
        ${h.map((u,x)=>{const v=y.length+x,S=ne[u.categorie]||"cat-alleg",C=Math.abs(parseFloat(u.montant_pat)),_=`${u.code}_alleg`;return P[_]={c:u,type:"alleg"},`
            <tr class="data-row" id="row-${v}" onclick="toggleExpl(${v})">
              <td>
                <span class="expand-icon">▶</span>
                <span class="cat ${S}">[${u.categorie}]</span>
                <span>${u.libelle}</span>
              </td>
              <td class="r">${d(u.base)}</td>
              <td class="r"></td>
              <td class="r"></td>
              <td class="r c-alleg">${z(Math.abs(parseFloat(u.taux_pat)))}</td>
              <td class="r c-alleg" onclick="event.stopPropagation();showFormula('${_}')" style="cursor:pointer">− ${d(C)}${q(_)}</td>
            </tr>
            <tr class="expl-row" id="expl-${v}" style="display:none">
              <td colspan="6">
                <div class="expl-box">
                  <div class="expl-txt">▸ ${m(u.explication)}</div>
                  ${u.loi_ref?`<div class="expl-ref">§ ${m(u.loi_ref)}</div>`:""}
                </div>
              </td>
            </tr>`}).join("")}
        <tr class="tbl-total">
          <td colspan="5">TOTAL ALLÈGEMENTS PATRONAUX</td>
          <td class="r c-alleg">− ${d(Math.abs(U))}</td>
        </tr>
      </tbody>
    </table>`;t.innerHTML=L+g+`<div class="tbl-wrap">${$}${b}</div>`}window.mobToggle=function(e,t){const l=document.getElementById("mob-expand-"+e);if(!l)return;const s=["why","how","sal","pat"],a=l.style.display!=="none",n=l.dataset.panel,i=o=>{const r=document.getElementById(`mob-expand-${e}-${o}`);r&&(r.style.display=o===t?"block":"none")};a?n===t?l.style.display="none":(l.dataset.panel=t,s.forEach(i)):(l.style.display="block",l.dataset.panel=t,s.forEach(i))};function Ne(e,t,l,s,a,n=0){const i=e.code==="REDUCTION_FILLON"?`<pre class="fm-fillon">${m(e.explication)}</pre>`:`<div class="fm-type-${a}">${k(e,a)}</div>`,o=`
    ...
    </div>`}function Re(e){const t=document.getElementById("res-mobile"),l=document.getElementById("m-nom")?.value||document.getElementById("d-nom")?.value||"",s=document.getElementById("m-prenom")?.value||document.getElementById("d-prenom")?.value||"",a=e.cotisations,n=["suisse","luxembourg"].includes(e.salarie?.pays),i=a.reduce((b,u)=>b+parseFloat(u.montant_sal),0),o=a.reduce((b,u)=>b+parseFloat(u.montant_pat),0),r=n?{total:0,taux_effectif:0}:Z(e.net_imposable),f=parseFloat(e.net_a_payer)-r.total,p=parseFloat(e.brut)+o,c=e.salarie?.pays==="suisse"?a.find(b=>b.code==="CH_IS"):null,g=c?parseFloat(c.montant_sal):0,y=c?parseFloat(c.taux_sal):0,h=i-g,I=a.filter(b=>b.categorie!=="Allègement"&&b.code!=="CH_IS"&&(parseFloat(b.montant_sal)>0||b.taux_sal!=="0"||parseFloat(b.montant_pat)>0)),w=a.filter(b=>b.categorie==="Allègement"),E=I.reduce((b,u)=>b+parseFloat(u.montant_pat),0),$=w.reduce((b,u)=>b+parseFloat(u.montant_pat),0),L=I.map((b,u)=>{const x=parseFloat(b.montant_sal)>0,v=parseFloat(b.montant_pat)>0,S=`${b.code}_u`,C=b.code==="REDUCTION_FILLON",_=x?C?`<pre class="fm-fillon">${m(b.explication)}</pre>`:`<div class="fm-type-sal">${k(b,"sal")}</div>`:"",j=v?C?`<pre class="fm-fillon">${m(b.explication)}</pre>`:`<div class="fm-type-pat">${k(b,"pat")}</div>`:"",V=`
      <div class="mob-exp-txt">${m(b.explication)}</div>
      ${b.loi_ref?`<div class="mob-exp-loi">§ ${m(b.loi_ref)}</div>`:""}`,B=`mob-stripe-sal-${u%2===0?"a":"b"}`,F=x?`<span class="mob-val mob-cot-amt" style="color:#ffe033" onclick="mobToggle('${S}','sal')">− ${d(b.montant_sal)}</span>`:`<span class="mob-val c-dim">0 ${H==="CHF"?"CHF":"€"}</span>`,N=v?`<span class="mob-val c-orange mob-cot-amt" onclick="mobToggle('${S}','pat')">+ ${d(b.montant_pat)}</span>`:`<span class="mob-val c-dim">0 ${H==="CHF"?"CHF":"€"}</span>`;return`
      <div class="${B}">
        <div class="mob-row">
          <span class="mob-lbl mob-cot-lbl" onclick="mobToggle('${S}','why')">${m(b.libelle)}</span>
          <span style="display:flex;flex-direction:column;align-items:flex-end;gap:0.1rem">${F}${N}</span>
        </div>
        <div class="mob-expand" id="mob-expand-${S}" style="display:none">
          <div id="mob-expand-${S}-why">${V}</div>
          ${_?`<div id="mob-expand-${S}-sal" style="display:none">${_}</div>`:""}
          ${j?`<div id="mob-expand-${S}-pat" style="display:none">${j}</div>`:""}
        </div>
      </div>`}).join(""),U=w.map((b,u)=>Ne(b,`${b.code}_alleg`,`− ${d(Math.abs(parseFloat(b.montant_pat)))}`,"c-alleg","alleg",u)).join("");t.innerHTML=`
    <div class="mob-bulletin">
      ...
      <div class="mob-head">
        ...
        <div style="text-align:right">
          ...
          <div class="mob-head-date">simulation au ${K(be())}</div>
        </div>
      </div>
      ...
      <div class="mob-row subtot">
        ...
        <span class="mob-val c-red">− ${d(h)}</span>
      </div>
      <div class="mob-row subtot">
        <span class="mob-lbl">TOTAL charges patronales</span>
        <span class="mob-val c-orange">+ ${d(E)}</span>
      </div>
      ...
      ${c?`<div class="mob-row pas-row" style="cursor:pointer" onclick="togglePasDetail('is-detail-mob')">
        ...
        <span class="mob-val c-purple">− ${d(g)}</span>
      </div>
      <div id="is-detail-mob" style="display:none;padding:0.4rem 0.6rem 0.2rem">
        <div class="fm-type-sal">${k(c,"sal")}</div>
        <div class="mob-exp-txt" style="margin-top:0.5rem">${m(c.explication)}</div>
        ${c.loi_ref?`<div class="mob-exp-loi">§ ${m(c.loi_ref)}</div>`:""}
      </div>`:""}
      ...
      ${n?"":`<div class="mob-row pas-row" style="cursor:pointer" onclick="togglePasDetail('pas-detail-mob')">
        <span class="mob-lbl">Prélèvement à la source (${(r.taux_effectif*100).toFixed(1)} %) <span id="pas-detail-mob-arrow" style="font-size:0.65em">▶</span></span>
        <span class="mob-val c-purple">− ${d(r.total)}</span>
      </div>
      <div id="pas-detail-mob" class="fm-type-pas" style="display:none;padding:0.4rem 0.6rem 0.2rem">
        ${fe(parseFloat(e.net_imposable))}
      </div>`}

      <!-- Net à payer -->
      <div class="mob-row final-row">
        <span class="mob-lbl">NET À PAYER</span>
        <span class="mob-val c-green">${d(f)}</span>
      </div>
      ...
      <div class="mob-row subtot">
        ...
        <span class="mob-val c-alleg">− ${d(Math.abs($))}</span>
      </div>`:""}

      <!-- Super brut -->
      <div class="mob-row superbrut">
        <span class="mob-lbl">SUPER BRUT (coût employeur)</span>
        <span class="mob-val c-blue">${d(p)}</span>
      </div>

    </div>`}function ge(e){H=e.devise||"EUR",Fe(e),Re(e),A&&Ae()}function ce(e){const t=`<div style="padding:1.5rem;color:#f87171;font-size:0.8rem">⚠ ${m(e)}</div>`;document.getElementById("res-desktop").innerHTML=t,document.getElementById("res-mobile").innerHTML=t}async function ve(e){const t=e==="mobile",l=document.getElementById(t?"m-brut":"d-brut").value,s=document.getElementById(t?"m-statut":"d-statut").value,a=document.getElementById(t?"m-nom":"d-nom").value||"Dupont",n=document.getElementById(t?"m-prenom":"d-prenom").value||"Marie",i=document.getElementById(t?"m-date":"d-date").value||pe,o=document.getElementById(t?"m-alsace-moselle":"d-alsace-moselle")?.checked??!1,r=document.getElementById(t?"m-suisse":"d-suisse")?.checked??!1,f=document.getElementById(t?"m-luxembourg":"d-luxembourg")?.checked??!1,p=document.getElementById(t?"m-fpt":"d-fpt")?.checked??!1,c=document.getElementById(t?"m-assujetti-is":"d-assujetti-is")?.checked??!1,g=document.getElementById(t?"m-canton":"d-canton")?.value||null,y=document.getElementById(t?"m-tarif-is":"d-tarif-is")?.value||null,h=parseFloat(l);if(!l||isNaN(h)||h<=0){ce("Salaire brut invalide — saisir un montant positif.");return}if(!/^\d{4}-\d{2}-\d{2}$/.test(i)){ce(`Date invalide : '${i}' (format attendu : YYYY-MM-DD).`);return}["d-brut","m-brut"].forEach(E=>{const $=document.getElementById(E);$&&($.value=l)}),["d-statut","m-statut"].forEach(E=>{const $=document.getElementById(E);$&&($.value=s)}),["d-nom","m-nom"].forEach(E=>{const $=document.getElementById(E);$&&($.value=a)}),["d-prenom","m-prenom"].forEach(E=>{const $=document.getElementById(E);$&&($.value=n)}),["d-date","m-date"].forEach(E=>{const $=document.getElementById(E);$&&($.value=i)});const I=r?"suisse":f?"luxembourg":null,w=I?"2026-01-01":i;try{const E=await me("calculer_bulletin",{salarie:{nom:a,prenom:n,salaire_brut:l.toString(),statut:s,alsace_moselle:o,pays:I??(p?"fonction_publique":"france"),assujetti_is:c,canton:r&&c&&g?g:null,tarif_is:r&&c&&y?y:null},datePaie:w});X=E,ge(E)}catch(E){console.error("[calculer_bulletin] erreur brute :",E);const $=D(E),L=`<div style="padding:1.5rem;color:#f87171;font-size:0.8rem">ERREUR : ${m($)}</div>`;document.getElementById("res-desktop").innerHTML=L,document.getElementById("res-mobile").innerHTML=L}}function qe(e){const t=document.getElementById("res-annuel"),l=e.lignes,s=l.map(p=>p.smic),a=`
    <thead><tr>
      ...
      <th>NET</th>
      <th>COÛT EMPL.</th>
    </tr></thead>`,n=l.map((p,c)=>{const g=c>0&&p.smic!==s[c-1],y=p.mois_libelle.includes("13e"),h=parseFloat(p.fillon_regularise)-parseFloat(p.fillon_simple),I=Math.abs(h)<.005?'<span style="color:var(--dim)">—</span>':`<span class="delta-nonzero">${h>0?"+":""}${Te(h.toFixed(2))}</span>`;return`<tr class="${[g?"smic-change":"",y?"treizieme-mois":""].filter(Boolean).join(" ")}">
      <td>${p.mois_libelle}</td>
      <td>${d(p.smic)}</td>
      <td>${d(p.brut)}</td>
      <td class="c-sal">− ${d(p.total_sal)}</td>
      <td class="c-pat">+ ${d(p.total_pat_brut)}</td>
      <td class="c-alleg">− ${d(p.fillon_regularise)}</td>
      <td>${I}</td>
      <td class="c-green">${d(p.net_a_payer)}</td>
      <td class="c-yellow">${d(p.cout_employeur)}</td>
    </tr>`}).join(""),i=`
    <tr class="ann-total">
      <td>TOTAL ${e.annee}</td>
      <td></td>
    ...
    </tr>`,o=parseFloat(e.total_pat_brut),r=parseFloat(e.total_fillon),f=`
    <div style="display:flex;gap:1rem;flex-wrap:wrap;margin-top:0.75rem;font-size:0.72rem">
      ...
      <div style="border:1px solid var(--border);padding:0.5rem 0.9rem;background:var(--bg3)">
        ...
        <div style="color:var(--blue);font-size:1.1rem;font-weight:bold">
          ${o>0?(r/parseFloat(e.total_brut)*100).toFixed(2)+" %":"—"}
        </div>
      </div>
      <div style="border:1px solid var(--border);padding:0.5rem 0.9rem;background:var(--bg3)">
    ...
    </div>`}async function Ue(e){forgeNav("detail");const t=document.getElementById("forge-profil-content");t.innerHTML='<div style="color:var(--muted);font-size:0.75rem">chargement…</div>';try{let l=T.find(s=>s.pseudo.toLowerCase()===e.toLowerCase());if(!l){const s=await fetch(`/profil/${encodeURIComponent(e)}`);if(!s.ok)throw new Error(`HTTP ${s.status} — ${await s.text()||s.statusText}`);l=await s.json()}t.innerHTML=je(l)}catch(l){t.innerHTML=`<div style="color:var(--red);font-size:0.75rem">Erreur : ${m(D(l))}</div>`}}function je(e){const t=e.linkedin_url?`<a class="profil-linkedin" href="${m(e.linkedin_url)}" target="_blank" rel="noopener noreferrer">↗ LinkedIn</a>`:"",s=[{niveau:"Maîtrisée",cls:"m",items:e.expertises.filter(i=>i.niveau==="Maîtrisée")},{niveau:"Pratiquée",cls:"p",items:e.expertises.filter(i=>i.niveau==="Pratiquée")},{niveau:"Connue",cls:"c",items:e.expertises.filter(i=>i.niveau==="Connue")}].filter(i=>i.items.length>0).map(i=>`
    ...
    </tr>`).join("")}`).join(""),a=e.expertises.length===0?'<div style="color:var(--muted);font-size:0.72rem">Aucune CCN renseignée.</div>':`<table class="profil-ccn-tbl">${s}</table>`,n=e.created_at?K(e.created_at.slice(0,10)):"—";return`
    ...
    </div>`}window.setPosteType=function(e){document.getElementById("poste_est_actuel_input").value=e?"1":"0",document.getElementById("ptog-actuel").className="ptog "+(e?"ptog-on":"ptog-off"),document.getElementById("ptog-vise").className="ptog "+(e?"ptog-off":"ptog-on")};const he=[{code:"BE",libelle:"Belgique"},{code:"LU",libelle:"Luxembourg"},{code:"DE",libelle:"Allemagne"},{code:"CH",libelle:"Suisse"},{code:"IT",libelle:"Italie"},{code:"MC",libelle:"Monaco"},{code:"ES",libelle:"Espagne"},{code:"AD",libelle:"Andorre"},{code:"GB",libelle:"Royaume-Uni"}],ze=he.map(e=>`<option value="${e.code}">${m(e.libelle)}</option>`).join("");let Ee=0;window.forgeAjouterPays=function(){const e=++Ee,t=document.createElement("div");t.className="forge-ccn-row",t.id="forge-pays-"+e,t.innerHTML=`
    <select class="forge-pays-select">${ze}</select>
    ...
    <button type="button" class="forge-ccn-del" onclick="forgeSupprPays(${e})" title="Supprimer">×</button>`,document.getElementById("forge-pays-list").appendChild(t)};window.forgeSupprPays=function(e){document.getElementById("forge-pays-"+e)?.remove()};let $e=0;window.forgeAjouterCcn=function(){const e=++$e,t=document.createElement("div");t.className="forge-ccn-row",t.id="forge-ccn-"+e,t.innerHTML=`
    <select class="forge-ccn-select">${Oe}</select>
    <select class="forge-ccn-niveau">
      <option value="Connue">Connue</option>
      <option value="Pratiquée">Pratiquée</option>
      <option value="Maîtrisée" selected>Maîtrisée</option>
    </select>
    <button type="button" class="forge-ccn-del" onclick="forgeSupprCcn(${e})" title="Supprimer">×</button>`,document.getElementById("forge-ccn-list").appendChild(t)};window.forgeSupprCcn=function(e){document.getElementById("forge-ccn-"+e)?.remove()};window.forgeSoumettre=async function(e){e.preventDefault();const t=document.getElementById("forge-form"),l=document.getElementById("forge-form-err"),s=document.getElementById("forge-submit-btn");l.textContent="";const a=[];document.querySelectorAll('[id^="forge-pays-"]').forEach(o=>{const r=o.querySelector(".forge-pays-select")?.value,f=o.querySelector(".forge-ccn-niveau")?.value,p=he.find(c=>c.code===r);r&&p&&a.push({pays_code:r,pays_libelle:p.libelle,niveau:f})});const n=[];document.querySelectorAll('.forge-ccn-row:not([id^="forge-pays-"])').forEach(o=>{const r=o.querySelector(".forge-ccn-select").value,f=o.querySelector(".forge-ccn-niveau").value,p=ye.find(c=>c.idcc===r);r&&p&&n.push({ccn_idcc:r,ccn_libelle:p.libelle,niveau:f})});const i={email:t.querySelector('[name="email"]').value.trim(),pseudo:t.querySelector('[name="pseudo"]').value.trim(),poste:t.querySelector('[name="poste"]').value.trim(),linkedin_url:t.querySelector('[name="linkedin_url"]').value.trim()||null,poste_est_actuel:t.querySelector('[name="poste_est_actuel"]').value!=="0",paie_fr_niveau:t.querySelector('[name="paie_fr_niveau"]').value||null,pays:a,expertises:n};if(!i.email){l.textContent="Email requis.";return}if(!i.pseudo){l.textContent="Pseudo requis.";return}if(!i.poste){l.textContent="Poste requis.";return}s.disabled=!0,s.textContent="[ envoi… ]";try{const o=await fetch("/forge/profil",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(!o.ok)throw new Error(`HTTP ${o.status} — ${await o.text()||o.statusText}`);const r=await o.json();T.unshift(r),t.reset(),document.getElementById("forge-pays-list").innerHTML="",document.getElementById("forge-ccn-list").innerHTML="",Ee=0,$e=0,Ue(r.pseudo)}catch(o){l.textContent=D(o),s.disabled=!1,s.textContent="[ Rejoindre la Forge ]"}};const Ge=[{prenom:"Geralt",nom:"de Riv"},{prenom:"Sam",nom:"Vimes"},{prenom:"Elric",nom:"de Melniboné"},{prenom:"Druss",nom:"la Légende"},{prenom:"Logen",nom:"Neuf-Doigts"},{prenom:"Aragorn",nom:"Grands-Pas"},{prenom:"Jon",nom:"Shannow"},{prenom:"Salim",nom:"Dhibi"},{prenom:"Bayaz",nom:"le Magi"},{prenom:"Merlin",nom:"l'Enchanteur"}],Ve=[{prenom:"Lyra",nom:"Belacqua"},{prenom:"Hermione",nom:"Granger"},{prenom:"Eowyn",nom:"du Rohan"},{prenom:"Ellana",nom:"Caldin"},{prenom:"Ferro",nom:"Maljinn"},{prenom:"Magrat",nom:"Garlick"},{prenom:"Ewilan",nom:"Gil'Sayan"},{prenom:"Sigarni",nom:"la Guerrière"},{prenom:"Rikke",nom:"la Nord"},{prenom:"Tanaquil",nom:"la Magicienne"}],re=[17,16,16,15,15,15,14,14,14,13,13,11],O=re[Math.floor(Math.random()*re.length)]/100;let de="H",Q=!1;function ue(e){return e[Math.floor(Math.random()*e.length)]}function xe(e,t){["d-prenom","m-prenom"].forEach(l=>{const s=document.getElementById(l);s&&(s.value=e)}),["d-nom","m-nom"].forEach(l=>{const s=document.getElementById(l);s&&(s.value=t)}),Q=!1}function Ie(e,t=!1){const l=e==="H";["d-hf-h","m-hf-h"].forEach(s=>{document.getElementById(s)?.classList.toggle("ptog-on",l),document.getElementById(s)?.classList.toggle("ptog-off",!l)}),["d-hf-f","m-hf-f"].forEach(s=>{document.getElementById(s)?.classList.toggle("ptog-on",!l),document.getElementById(s)?.classList.toggle("ptog-off",l)}),t&&document.querySelectorAll(".genre-ecart-hint").forEach(s=>{s.textContent=l?s.dataset.textHf:s.dataset.textFh,s.style.display="inline"})}window.setGenre=function(e){if(e===de)return;if(!Q){const l=e==="F"?window._heroF:window._heroH;xe(l.prenom,l.nom)}const t=e==="F"?1-O:1/(1-O);["d-brut","m-brut"].forEach(l=>{const s=document.getElementById(l);s&&(s.value=Math.round(parseFloat(s.value)*t))}),de=e,Ie(e,!0)};const ee=document.getElementById("burger-btn"),G=document.getElementById("burger-menu");function Ye(){ee.classList.add("open"),G.classList.add("open")}window.closeBurger=function(){ee.classList.remove("open"),G.classList.remove("open")};ee.addEventListener("click",e=>{e.stopPropagation(),G.classList.contains("open")?closeBurger():Ye()});document.addEventListener("click",()=>closeBurger());G.addEventListener("click",e=>e.stopPropagation());
