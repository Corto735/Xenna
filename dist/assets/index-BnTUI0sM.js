(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))s(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function l(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function s(a){if(a.ep)return;a.ep=!0;const n=l(a);fetch(a.href,n)}})();const he="modulepreload",Ee=function(e){return"/"+e},K={},$e=function(t,l,s){let a=Promise.resolve();if(l&&l.length>0){let i=function(c){return Promise.all(c.map(r=>Promise.resolve(r).then(f=>({status:"fulfilled",value:f}),f=>({status:"rejected",reason:f}))))};document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),b=o?.nonce||o?.getAttribute("nonce");a=i(l.map(c=>{if(c=Ee(c),c in K)return;K[c]=!0;const r=c.endsWith(".css"),f=r?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${f}`))return;const v=document.createElement("link");if(v.rel=r?"stylesheet":he,r||(v.as="script"),v.crossOrigin="",v.href=c,b&&v.setAttribute("nonce",b),document.head.appendChild(v),r)return new Promise(($,h)=>{v.addEventListener("load",$),v.addEventListener("error",()=>h(new Error(`Unable to preload CSS for ${c}`)))})}))}function n(i){const o=new Event("vite:preloadError",{cancelable:!0});if(o.payload=i,window.dispatchEvent(o),!o.defaultPrevented)throw i}return a.then(i=>{for(const o of i||[])o.status==="rejected"&&n(o.reason);return t().catch(n)})};async function ne(e,t={}){if(window.__TAURI_INTERNALS__){const{invoke:s}=await $e(async()=>{const{invoke:a}=await import("./core-DV6XEvTN.js");return{invoke:a}},[]);return s(e,t)}const l=await fetch(`/api/${e}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)});if(!l.ok)throw await l.text();return l.json()}function B(e){if(e==null)return"(erreur nulle — redémarre l'app ou ouvre DevTools Ctrl+Shift+I)";if(typeof e=="string")return e||"(erreur muette — ouvre DevTools Ctrl+Shift+I et consulte la Console)";if(e instanceof Error)return e.message||e.toString();try{return JSON.stringify(e,null,2)}catch{return String(e)}}let j=null;const z="2026-01-31",oe=z;document.addEventListener("DOMContentLoaded",()=>{["d-date","m-date"].forEach(n=>{const i=document.getElementById(n);i&&(i.value=z,i.max=z)}),document.addEventListener("keydown",n=>{n.key==="Escape"&&closeFmModal()}),window._heroH=Math.random()<.015?{prenom:"Jean-Noël",nom:"Favari"}:ae(qe),window._heroF=ae(ke),fe(window._heroH.prenom,window._heroH.nom),ge("H"),["d-prenom","m-prenom","d-nom","m-nom"].forEach(n=>{document.getElementById(n)?.addEventListener("input",()=>{J=!0})});const e=Math.round(M*100),t=Math.round(M/(1-M)*100);document.querySelectorAll(".genre-ecart-hint").forEach(n=>{n.dataset.textFh=`// −${e} % · écart salarial F/H`,n.dataset.textHf=`// +${t} % · écart salarial H/F`});const l=window.matchMedia("(max-width: 680px)"),s=n=>{const i=document.body;!i.classList.contains("is-annuel")&&!i.classList.contains("is-forge")&&!i.classList.contains("is-apropos")&&!i.classList.contains("is-gaabrielle")&&!i.classList.contains("is-hercule")&&setView(n.matches?"mobile":"desktop")};l.addEventListener("change",s),s(l),localStorage.getItem("xenna-hv")&&(document.body.classList.add("hv-mode"),document.getElementById("hv-switch")?.classList.add("on")),localStorage.getItem("xenna-zoom")&&(document.body.classList.add("zoom-mode"),document.documentElement.style.zoom="200%",document.getElementById("zoom-switch")?.classList.add("on"),document.getElementById("a11y-magnifier")?.classList.add("active"));const a=localStorage.getItem("xenna-font");a&&setAppFont(a,!0),localStorage.getItem("xenna-hv")&&document.getElementById("a11y-hv-btn")?.classList.add("active"),localStorage.getItem("xenna-bw")&&(document.body.classList.add("bw-mode"),document.getElementById("bw-switch")?.classList.add("on"),document.getElementById("a11y-bw-btn")?.classList.add("active")),document.addEventListener("click",n=>{!n.target.closest("#a11y-btn")&&!n.target.closest("#a11y-panel")&&(document.getElementById("a11y-panel")?.classList.remove("open"),document.getElementById("a11y-btn")?.classList.remove("open"))})});let Z="fr";const D={},w=new Map;function xe(){const e="script,style,input,select,textarea,.mob-val,.sb-val,.fm-val,.a11y-float,.trad-panel,#a11y-panel",t=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT,{acceptNode(s){const a=s.textContent.trim();return!a||a.length<2||/^[\d\s,.\-+%€×\/:()[\]]+$/.test(a)||s.parentElement?.closest(e)?NodeFilter.FILTER_REJECT:NodeFilter.FILTER_ACCEPT}}),l=[];for(;t.nextNode();)l.push(t.currentNode);return l}window.toggleTradPanel=function(){const e=document.getElementById("trad-panel"),t=document.getElementById("trad-btn"),l=e.classList.toggle("open");t.classList.toggle("open",l)};window.translateApp=async function(e){if(document.getElementById("trad-panel")?.classList.remove("open"),document.getElementById("trad-btn")?.classList.remove("open"),document.querySelectorAll(".trad-lang-btn").forEach(i=>i.classList.remove("active")),document.querySelector(`.trad-lang-btn[onclick="translateApp('${e}')"]`)?.classList.add("active"),e==="fr"){w.forEach((i,o)=>{o.isConnected&&(o.textContent=i)}),document.documentElement.lang="fr",Z="fr";return}const t=document.getElementById("trad-btn");t.classList.add("loading"),t.textContent="🌐 …";const l=xe();l.forEach(i=>{w.has(i)||w.set(i,i.textContent)});const s=l.map(i=>w.get(i));D[e]||(D[e]=new Map);const a=D[e],n=[...new Set(s)].filter(i=>!a.has(i));try{if(n.length>0)for(let o=0;o<n.length;o+=20){const b=n.slice(o,o+20),c=b.join(`

`),r=`https://api.mymemory.translated.net/get?q=${encodeURIComponent(c)}&langpair=fr|${e}`,f=await fetch(r);if(!f.ok)throw new Error("HTTP "+f.status);const $=(await f.json()).responseData.translatedText.split(`

`);b.forEach((h,I)=>a.set(h,$[I]??h))}l.forEach(i=>{const o=w.get(i);i.isConnected&&a.has(o)&&(i.textContent=a.get(o))}),document.documentElement.lang=e,Z=e}catch(i){console.error("Traduction échouée :",i),t.textContent="🌐 ✗",setTimeout(()=>{t.textContent="🌐 LANGUE",t.classList.remove("loading")},2e3);return}t.textContent="🌐 LANGUE",t.classList.remove("loading")};document.addEventListener("click",e=>{!e.target.closest("#trad-btn")&&!e.target.closest("#trad-panel")&&(document.getElementById("trad-panel")?.classList.remove("open"),document.getElementById("trad-btn")?.classList.remove("open"))});window.toggleA11yPanel=function(){const e=document.getElementById("a11y-panel"),t=document.getElementById("a11y-btn"),l=e.classList.toggle("open");t.classList.toggle("open",l)};window.toggleHVMode=function(){const e=document.body.classList.toggle("hv-mode");document.getElementById("hv-switch")?.classList.toggle("on",e),document.getElementById("a11y-hv-btn")?.classList.toggle("active",e),localStorage.setItem("xenna-hv",e?"1":"")};window.toggleZoom=function(){const e=document.body.classList.toggle("zoom-mode");document.documentElement.style.zoom=e?"200%":"",document.getElementById("zoom-switch")?.classList.toggle("on",e),document.getElementById("a11y-magnifier")?.classList.toggle("active",e),localStorage.setItem("xenna-zoom",e?"1":"")};const Q=new Set,Ie=new Set(["IBM Plex Mono","Fira Code","JetBrains Mono","Source Code Pro","Roboto Mono","Inconsolata"]);window.setAppFont=function(e,t=!1){if(!e){document.body.classList.remove("custom-font"),document.documentElement.style.removeProperty("--app-font"),localStorage.removeItem("xenna-font");const a=document.getElementById("font-picker");a&&(a.value="");return}if(!Ie.has(e))return;const l=e.replace(/ /g,"+");if(!Q.has(l)){const a=document.createElement("link");a.rel="stylesheet",a.href=`https://fonts.googleapis.com/css2?family=${l}&display=swap`,document.head.appendChild(a),Q.add(l)}document.documentElement.style.setProperty("--app-font",`'${e}', monospace`),document.body.classList.add("custom-font"),localStorage.setItem("xenna-font",e);const s=document.getElementById("font-picker");s&&t&&(s.value=e)};const _=[];window.scan67=function(){const e=Date.now();for(_.push(e);_.length&&e-_[0]>1500;)_.shift();const t=_.length>=3;t&&(_.length=0);const l=t?/42/:/67/,a=Array.from(document.querySelectorAll(".mob-val, .sb-val, .ascii-tbl td, .fm-val, .fm-result td")).filter(i=>l.test(i.textContent.replace(/[\s ]/g,""))&&i.offsetParent!==null);if(a.length===0)return;const n=document.getElementById("a11y-67-btn");if(n.classList.add("active"),t){const i=["#ff0055","#ff6600","#ffcc00","#00ff88","#00ccff","#aa00ff","#ff00cc","#39ff14","#ff4444","#44ffff","#ff69b4","#7fff00"];a.forEach((o,b)=>{setTimeout(()=>{const c=i[Math.floor(Math.random()*i.length)];Object.assign(o.style,{background:c,color:"#000",outline:`2px solid ${c}`,borderRadius:"2px",transition:"all 0.15s"}),setTimeout(()=>Object.assign(o.style,{background:"",color:"",outline:"",borderRadius:""}),900)},b*250)}),setTimeout(()=>n.classList.remove("active"),a.length*250+1e3)}else a.forEach((i,o)=>{setTimeout(()=>{i.classList.remove("flash-67"),i.offsetWidth,i.classList.add("flash-67"),i.addEventListener("animationend",()=>i.classList.remove("flash-67"),{once:!0})},o*500)}),setTimeout(()=>n.classList.remove("active"),a.length*500+200)};window.toggleBWMode=function(){const e=document.body.classList.toggle("bw-mode");document.getElementById("bw-switch")?.classList.toggle("on",e),document.getElementById("a11y-bw-btn")?.classList.toggle("active",e),localStorage.setItem("xenna-bw",e?"1":"")};function p(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}window.setView=function(e){["mobile","desktop","annuel","forge","apropos","gaabrielle","hercule"].forEach(t=>document.body.classList.toggle("is-"+t,e===t)),document.getElementById("btn-desk").classList.toggle("active",e==="desktop"),document.getElementById("btn-mob").classList.toggle("active",e==="mobile"),document.getElementById("btn-ann").classList.toggle("active",e==="annuel"),j&&(e==="desktop"||e==="mobile")&&re(j),e==="forge"&&Pe()};let A="EUR";function u(e){const t=parseFloat(e),l=A==="CHF"?" CHF":" €";return t.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+l}function Se(e,t=!1){const l=parseFloat(e),s=A==="CHF"?" CHF":" €",a=l.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+s;return t&&l>0?"+"+a:a}function R(e){return(parseFloat(e)*100).toFixed(2)+" %"}function ce(){const e=document.body.classList.contains("is-mobile")?"m-date":"d-date",t=e==="m-date"?"d-date":"m-date";return document.getElementById(e)?.value||document.getElementById(t)?.value||oe}function G(e){if(!e)return"—";const[t,l,s]=e.split("-");return`${s}/${l}/${t}`}const Le=[{min:0,max:1620,taux:0},{min:1620,max:1683,taux:.005},{min:1683,max:1791,taux:.013},{min:1791,max:1911,taux:.021},{min:1911,max:2042,taux:.029},{min:2042,max:2151,taux:.035},{min:2151,max:2294,taux:.041},{min:2294,max:2714,taux:.053},{min:2714,max:3107,taux:.075},{min:3107,max:3539,taux:.099},{min:3539,max:3983,taux:.119},{min:3983,max:4648,taux:.138},{min:4648,max:5574,taux:.158},{min:5574,max:6974,taux:.179},{min:6974,max:8711,taux:.2},{min:8711,max:12091,taux:.24},{min:12091,max:16376,taux:.28},{min:16376,max:25706,taux:.33},{min:25706,max:55062,taux:.38},{min:55062,max:1/0,taux:.43}];function V(e){const t=parseFloat(e);if(isNaN(t)||t<=0)return{total:0,taux_effectif:0,details:[]};let l=0;const s=[];for(const a of Le){if(t<=a.min)break;const i=+((a.max===1/0?t:Math.min(t,a.max))-a.min).toFixed(2),o=i*a.taux;if(s.push({min:a.min,max:a.max===1/0?null:a.max,taux:a.taux,base:i,montant:+o.toFixed(2)}),l+=o,a.max===1/0||t<=a.max)break}return{total:+l.toFixed(2),taux_effectif:t>0?l/t:0,details:s}}const ee={"Sécurité Sociale":"cat-ss","CSG/CRDS":"cat-csg","Retraite complémentaire":"cat-ret",Prévoyance:"cat-prev",Chômage:"cat-cho",Allègement:"cat-alleg","1er pilier":"cat-ss","Assurance chômage":"cat-cho","Assurance accidents":"cat-acc","Prévoyance maladie":"cat-prev","Prévoyance (LPP)":"cat-ret","Assurance pension":"cat-ret","Assurance maladie":"cat-ss","Assurance dépendance":"cat-prev","Mutualité des employeurs":"cat-ss"},q={},te={SS_VIEILLESSE_PLAF:"min(Salaire brut, Plafond Mensuel Sécurité Sociale — PMSS)",CHOMAGE:"min(Salaire brut, 4 × PMSS)",AGS:"min(Salaire brut, 4 × PMSS)",CSG_DEDUCTIBLE:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels (CSS art. L136-2)",CSG_NON_DEDUCTIBLE:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels",CRDS:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels",AGIRC_ARRCO_T1:"min(Salaire brut, PMSS)  — Tranche 1 (entre 0 et 1 PMSS)",AGIRC_ARRCO_CEG_T1:"min(Salaire brut, PMSS)  — Tranche 1",PREVOYANCE_CADRE_MIN:"min(Salaire brut, PMSS)  — Tranche A",AGIRC_ARRCO_T2:"Fraction du salaire entre 1 PMSS et 8 PMSS  — Tranche 2",AGIRC_ARRCO_CEG_T2:"Fraction du salaire entre 1 PMSS et 8 PMSS  — Tranche 2"};function U(e,t=!0){const l=["f","(","x",")"].map((s,a)=>`<span style="animation-delay:${a*45}ms">${s}</span>`).join("");return t?`<span class="formula-star" data-fmkey="${e}" onclick="event.stopPropagation();showFormula('${e}')">${l}</span>`:`<span class="formula-star" aria-hidden="true">${l}</span>`}window.togglePasDetail=function(e){const t=document.getElementById(e);if(!t)return;const l=t.style.display!=="none";t.style.display=l?"none":"block";const s=document.getElementById(e+"-arrow");s&&(s.textContent=l?"▶":"▼")};function k(e,t){if(e.code==="REDUCTION_FILLON")return`<pre class="fm-fillon">${p(e.explication)}</pre>`;const l=t==="sal",s=l?e.taux_sal:e.taux_pat,a=parseFloat(e.base),n=l?parseFloat(e.montant_sal):Math.abs(parseFloat(e.montant_pat)),i=l?"Taux salarial":"Taux patronal",o=l?"Montant salarial":t==="alleg"?"Montant allègement":"Montant patronal",b=l?"c-sal":t==="alleg"?"c-alleg":"c-pat",c=te[e.code]?`<div class="fm-base-note">Assiette  =  ${p(te[e.code])}</div>`:"";return`
    ...
    <table class="fm-calc">
      <tr>
        <td>Assiette</td>
        <td class="fm-op">=</td>
        <td class="fm-val c-base">${u(a)}</td>
      </tr>
      <tr>
        <td>${i}</td>
        <td class="fm-op">×</td>
        <td class="fm-val c-taux">${R(s)}</td>
      </tr>
      <tr class="fm-result fm-sep">
        <td>${o}</td>
        <td class="fm-op">=</td>
        <td class="fm-val ${b}">${u(n)}</td>
      </tr>
    </table>`}function Y(e){const t=V(e);return`
    ...
    <table class="pas-tbl">
      ...
      <tr class="${i?"pas-zero":""}">
        <td>${a} → ${n}</td>
        <td class="r">${u(s.base)}</td>
        <td class="r ${i?"c-dim":""}">${(s.taux*100).toFixed(1).replace(".",",")} %</td>
        <td class="r ${i?"c-dim":"c-purple"}">${i?"—":u(s.montant)}</td>
      </tr>`}).join("")}</tbody>
      <tfoot>
        <tr>
          <td>Net imposable</td>
          <td class="r c-gray">${u(e)}</td>
          <td class="r c-taux">${(t.taux_effectif*100).toFixed(2)} %&nbsp;<span style="color:var(--dim);font-size:0.7em">(taux effectif)</span></td>
          <td class="r c-purple" style="font-weight:bold">${u(t.total)}</td>
        </tr>
      </tfoot>
    </table>`}window.showFormula=function(e){const t=q[e];if(!t)return;const l=document.getElementById("fm-body");if(t.type==="pas"){document.getElementById("fm-title").textContent="Prélèvement à la Source (PAS)",document.getElementById("fm-badge").textContent="── Détail par tranche — barème neutre mensuel DGFIP ─────────",l.className="fm-type-pas",l.innerHTML=Y(t.netImposable),document.getElementById("fm-modal").classList.add("open"),document.querySelectorAll(`[data-fmkey="${e}"]`).forEach(o=>o.classList.add("visited"));return}const{c:s,type:a}=t,n=a==="sal",i=s.code==="REDUCTION_FILLON"?"── Allègement patronal ──────────────────────":n?"── Part salariale ───────────────────────────":"── Part patronale ───────────────────────────";document.getElementById("fm-title").textContent=s.libelle,document.getElementById("fm-badge").textContent=i,l.className=`fm-type-${a}`,l.innerHTML=k(s,a),document.getElementById("fm-modal").classList.add("open"),document.querySelectorAll(`[data-fmkey="${e}"]`).forEach(o=>o.classList.add("visited"))};window.closeFmModal=function(){document.getElementById("fm-modal").classList.remove("open")};window.toggleExpl=function(e){const t=document.getElementById(`row-${e}`),l=document.getElementById(`expl-${e}`);if(!t||!l)return;const s=l.style.display!=="none";l.style.display=s?"none":"table-row",t.classList.toggle("open",!s)};function _e(e){const t=document.getElementById("res-desktop"),l=e.cotisations,s=e.salarie?.pays&&e.salarie.pays!=="france",a=l.reduce((m,E)=>m+parseFloat(E.montant_sal),0),n=l.reduce((m,E)=>m+parseFloat(E.montant_pat),0),i=s?{total:0,taux_effectif:0}:V(e.net_imposable),o=parseFloat(e.net_a_payer)-i.total,b=`
    <div class="summary-bar">
      <div class="sb-cell">
        <div class="sb-lbl">▸ SALAIRE BRUT</div>
        <div class="sb-val c-gray">${u(e.brut)}</div>
      </div>
      <div class="sb-cell">
        ...
        <div class="sb-ded">
          <div class="sb-ded-row">
            ...
            <span style="color:var(--red)">− ${u(a)}</span>
          </div>
          ${s?"":`<div class="sb-ded-row" style="cursor:pointer" onclick="togglePasDetail('pas-detail-desk')">
            <span>PAS (${(i.taux_effectif*100).toFixed(1)} %) <span id="pas-detail-desk-arrow" style="font-size:0.65em;color:var(--muted)">▶</span></span>
            <span style="color:var(--purple)">− ${u(i.total)}</span>
          </div>
          <div id="pas-detail-desk" style="display:none;margin-top:0.3rem">
            ${Y(parseFloat(e.net_imposable))}
          </div>`}
          <div class="sb-ded-total">
            <span>Total retenues</span>
            <span style="color:var(--red)">− ${u(a+i.total)}</span>
          </div>
        </div>
      </div>
      <div class="sb-cell">
        <div class="sb-lbl">▸ NET À PAYER</div>
        <div class="sb-val c-green">${u(o)}</div>
      </div>
      <div class="sb-cell">
        <div class="sb-lbl">▸ CHARGES PAT.</div>
        <div class="sb-val c-orange">${u(n)}</div>
      </div>
      <div class="sb-cell">
        <div class="sb-lbl">▸ SUPER BRUT</div>
        <div class="sb-val c-yellow">${u(parseFloat(e.brut)+n)}</div>
      </div>
    </div>`,c=l.filter(m=>m.categorie!=="Allègement"&&(parseFloat(m.montant_sal)>0||m.taux_sal!=="0"||parseFloat(m.montant_pat)>0)),r=l.filter(m=>m.categorie==="Allègement"),f=c.reduce((m,E)=>m+parseFloat(E.montant_pat),0);function v(m,E){return m.map((g,x)=>{const S=E+x,L=ee[g.categorie]||"cat-ss",P=parseFloat(g.montant_sal)>0?"c-sal":"c-dim",H=parseFloat(g.montant_pat)>0?"c-pat":"c-dim",T=`${g.code}_sal`,F=`${g.code}_pat`,N=parseFloat(g.montant_sal)>0,W=parseFloat(g.montant_pat)>0;N&&(q[T]={c:g,type:"sal"}),W&&(q[F]={c:g,type:"pat"});const ve=U(T,N),ye=U(F,W);return`
        <tr class="data-row" id="row-${S}" onclick="toggleExpl(${S})">
          <td>
            <span class="expand-icon">▶</span>
            <span class="cat ${L}">[${g.categorie}]</span>
            <span>${g.libelle}</span>
          </td>
          <td class="r">${u(g.base)}</td>
          <td class="r">${R(g.taux_sal)}</td>
          <td class="r ${P}">${u(g.montant_sal)}${ve}</td>
          <td class="r">${R(g.taux_pat)}</td>
          <td class="r ${H}">${u(g.montant_pat)}${ye}</td>
        </tr>
        <tr class="expl-row" id="expl-${S}" style="display:none">
          <td colspan="6">
            <div class="expl-box">
              <div class="expl-txt">▸ ${p(g.explication)}</div>
              ${g.loi_ref?`<div class="expl-ref">§ ${p(g.loi_ref)}</div>`:""}
            </div>
          </td>
        </tr>`}).join("")}const $=`
    <colgroup>
    ...
    <table class="ascii-tbl">
      ...
      <tbody>
        ...
        <tr class="tbl-total">
          ...
          <td class="r c-sal">− ${u(a)}</td>
          <td></td>
          <td class="r c-pat">+ ${u(f)}</td>
        </tr>
      </tbody>
    </table>`,I=`<div class="sim-period">
    SIMULATION AU <span class="sp-accent">${G(ce())}</span>
    ...
    <table class="ascii-tbl">
      ${$}
      <tbody>
        ${r.map((m,E)=>{const g=c.length+E,x=ee[m.categorie]||"cat-alleg",S=Math.abs(parseFloat(m.montant_pat)),L=`${m.code}_alleg`;return q[L]={c:m,type:"alleg"},`
            <tr class="data-row" id="row-${g}" onclick="toggleExpl(${g})">
              <td>
                <span class="expand-icon">▶</span>
                <span class="cat ${x}">[${m.categorie}]</span>
                <span>${m.libelle}</span>
              </td>
              <td class="r">${u(m.base)}</td>
              <td class="r"></td>
              <td class="r"></td>
              <td class="r c-alleg">${R(Math.abs(parseFloat(m.taux_pat)))}</td>
              <td class="r c-alleg">− ${u(S)}${U(L)}</td>
            </tr>
            <tr class="expl-row" id="expl-${g}" style="display:none">
              <td colspan="6">
                <div class="expl-box">
                  <div class="expl-txt">▸ ${p(m.explication)}</div>
                  ${m.loi_ref?`<div class="expl-ref">§ ${p(m.loi_ref)}</div>`:""}
                </div>
              </td>
            </tr>`}).join("")}
        <tr class="tbl-total">
          <td colspan="5">TOTAL ALLÈGEMENTS PATRONAUX</td>
          <td class="r c-alleg">− ${u(Math.abs(y))}</td>
        </tr>
      </tbody>
    </table>`;t.innerHTML=I+b+`<div class="tbl-wrap">${h}${d}</div>`}window.mobToggle=function(e,t){const l=document.getElementById("mob-expand-"+e);if(!l)return;const s=["why","how","sal","pat"],a=l.style.display!=="none",n=l.dataset.panel,i=o=>{const b=document.getElementById(`mob-expand-${e}-${o}`);b&&(b.style.display=o===t?"block":"none")};a?n===t?l.style.display="none":(l.dataset.panel=t,s.forEach(i)):(l.style.display="block",l.dataset.panel=t,s.forEach(i))};function Ce(e,t,l,s,a,n=0){const i=e.code==="REDUCTION_FILLON"?`<pre class="fm-fillon">${p(e.explication)}</pre>`:`<div class="fm-type-${a}">${k(e,a)}</div>`,o=`
    <div class="mob-exp-txt">${p(e.explication)}</div>
    ${e.loi_ref?`<div class="mob-exp-loi">§ ${p(e.loi_ref)}</div>`:""}`;return`
    <div class="${`mob-stripe-${a}-${n%2===0?"a":"b"}`}">
      <div class="mob-row">
        <span class="mob-lbl mob-cot-lbl"
              title="Explication et référence légale"
              onclick="mobToggle('${t}','why')">${p(e.libelle)}</span>
    ...
    </div>`}function we(e){const t=document.getElementById("res-mobile"),l=document.getElementById("m-nom")?.value||document.getElementById("d-nom")?.value||"",s=document.getElementById("m-prenom")?.value||document.getElementById("d-prenom")?.value||"",a=e.cotisations,n=e.salarie?.pays&&e.salarie.pays!=="france",i=a.reduce((d,m)=>d+parseFloat(m.montant_sal),0),o=a.reduce((d,m)=>d+parseFloat(m.montant_pat),0),b=n?{total:0,taux_effectif:0}:V(e.net_imposable),c=parseFloat(e.net_a_payer)-b.total,r=parseFloat(e.brut)+o,f=a.filter(d=>d.categorie!=="Allègement"&&(parseFloat(d.montant_sal)>0||d.taux_sal!=="0"||parseFloat(d.montant_pat)>0)),v=a.filter(d=>d.categorie==="Allègement"),$=f.reduce((d,m)=>d+parseFloat(m.montant_pat),0),h=v.reduce((d,m)=>d+parseFloat(m.montant_pat),0),I=f.map((d,m)=>{const E=parseFloat(d.montant_sal)>0,g=parseFloat(d.montant_pat)>0,x=`${d.code}_u`,S=d.code==="REDUCTION_FILLON",L=E?S?`<pre class="fm-fillon">${p(d.explication)}</pre>`:`<div class="fm-type-sal">${k(d,"sal")}</div>`:"",P=g?S?`<pre class="fm-fillon">${p(d.explication)}</pre>`:`<div class="fm-type-pat">${k(d,"pat")}</div>`:"",H=`
      <div class="mob-exp-txt">${p(d.explication)}</div>
      ${d.loi_ref?`<div class="mob-exp-loi">§ ${p(d.loi_ref)}</div>`:""}`,T=`mob-stripe-sal-${m%2===0?"a":"b"}`,F=E?`<span class="mob-val mob-cot-amt" style="color:#ffe033" onclick="mobToggle('${x}','sal')">− ${u(d.montant_sal)}</span>`:`<span class="mob-val c-dim">0 ${A==="CHF"?"CHF":"€"}</span>`,N=g?`<span class="mob-val c-orange mob-cot-amt" onclick="mobToggle('${x}','pat')">+ ${u(d.montant_pat)}</span>`:`<span class="mob-val c-dim">0 ${A==="CHF"?"CHF":"€"}</span>`;return`
      <div class="${T}">
        <div class="mob-row">
          <span class="mob-lbl mob-cot-lbl" onclick="mobToggle('${x}','why')">${p(d.libelle)}</span>
        ...
        <div class="mob-expand" id="mob-expand-${x}" style="display:none">
          <div id="mob-expand-${x}-why">${H}</div>
          ${L?`<div id="mob-expand-${x}-sal" style="display:none">${L}</div>`:""}
          ${P?`<div id="mob-expand-${x}-pat" style="display:none">${P}</div>`:""}
        </div>
      </div>`}).join(""),y=v.map((d,m)=>Ce(d,`${d.code}_alleg`,`− ${u(Math.abs(parseFloat(d.montant_pat)))}`,"c-alleg","alleg",m)).join("");t.innerHTML=`
    <div class="mob-bulletin">
      ...
      <div class="mob-head">
        ...
        <div style="text-align:right">
          <div class="mob-head-name">${p(s)} ${p(l).toUpperCase()}</div>
          <div class="mob-head-date">simulation au ${G(ce())}</div>
        </div>
      </div>
      ...
      <div class="mob-row" style="margin-top:0.15rem">
        ...
        <span class="mob-val c-gray">${u(e.brut)}</span>
      </div>
      ...
      <div class="mob-row subtot">
        ...
        <span class="mob-val c-red">− ${u(i)}</span>
      </div>
      <div class="mob-row subtot">
        <span class="mob-lbl">TOTAL charges patronales</span>
        <span class="mob-val c-orange">+ ${u($)}</span>
      </div>

      <!-- Net imposable (France seulement) -->
      ${n?"":`<div class="mob-row net-row">
        <span class="mob-lbl">NET IMPOSABLE</span>
        <span class="mob-val c-green">${u(e.net_imposable)}</span>
      </div>`}

      <!-- PAS (France seulement) -->
      ${n?"":`<div class="mob-row pas-row" style="cursor:pointer" onclick="togglePasDetail('pas-detail-mob')">
        <span class="mob-lbl">Prélèvement à la source (${(b.taux_effectif*100).toFixed(1)} %) <span id="pas-detail-mob-arrow" style="font-size:0.65em">▶</span></span>
        <span class="mob-val c-purple">− ${u(b.total)}</span>
      </div>
      ...
      <div class="mob-row final-row">
        ...
        <span class="mob-val c-green">${u(c)}</span>
      </div>
      ...
      <div class="mob-row subtot">
        ...
        <span class="mob-val c-alleg">− ${u(Math.abs(h))}</span>
      </div>`:""}

      <!-- Super brut -->
      <div class="mob-row superbrut">
        <span class="mob-lbl">SUPER BRUT (coût employeur)</span>
        <span class="mob-val c-blue">${u(r)}</span>
      </div>

    </div>`}function re(e){A=e.devise||"EUR",_e(e),we(e)}function le(e){const t=`<div style="padding:1.5rem;color:#f87171;font-size:0.8rem">⚠ ${p(e)}</div>`;document.getElementById("res-desktop").innerHTML=t,document.getElementById("res-mobile").innerHTML=t}async function de(e){const t=e==="mobile",l=document.getElementById(t?"m-brut":"d-brut").value,s=document.getElementById(t?"m-statut":"d-statut").value,a=document.getElementById(t?"m-nom":"d-nom").value||"Dupont",n=document.getElementById(t?"m-prenom":"d-prenom").value||"Marie",i=document.getElementById(t?"m-date":"d-date").value||oe,o=document.getElementById(t?"m-alsace-moselle":"d-alsace-moselle")?.checked??!1,b=document.getElementById(t?"m-suisse":"d-suisse")?.checked??!1,c=document.getElementById(t?"m-luxembourg":"d-luxembourg")?.checked??!1,r=document.getElementById(t?"m-assujetti-is":"d-assujetti-is")?.checked??!1,f=document.getElementById(t?"m-canton":"d-canton")?.value||null,v=document.getElementById(t?"m-tarif-is":"d-tarif-is")?.value||null,$=parseFloat(l);if(!l||isNaN($)||$<=0){le("Salaire brut invalide — saisir un montant positif.");return}if(!/^\d{4}-\d{2}-\d{2}$/.test(i)){le(`Date invalide : '${i}' (format attendu : YYYY-MM-DD).`);return}["d-brut","m-brut"].forEach(y=>{const d=document.getElementById(y);d&&(d.value=l)}),["d-statut","m-statut"].forEach(y=>{const d=document.getElementById(y);d&&(d.value=s)}),["d-nom","m-nom"].forEach(y=>{const d=document.getElementById(y);d&&(d.value=a)}),["d-prenom","m-prenom"].forEach(y=>{const d=document.getElementById(y);d&&(d.value=n)}),["d-date","m-date"].forEach(y=>{const d=document.getElementById(y);d&&(d.value=i)});const h=b?"suisse":c?"luxembourg":null,I=h?"2026-01-01":i;try{const y=await ne("calculer_bulletin",{salarie:{nom:a,prenom:n,salaire_brut:l.toString(),statut:s,alsace_moselle:o,pays:h??"france",assujetti_is:r,canton:b&&r&&f?f:null,tarif_is:b&&r&&v?v:null},datePaie:I});j=y,re(y)}catch(y){console.error("[calculer_bulletin] erreur brute :",y);const d=B(y),m=`<div style="padding:1.5rem;color:#f87171;font-size:0.8rem">ERREUR : ${p(d)}</div>`;document.getElementById("res-desktop").innerHTML=m,document.getElementById("res-mobile").innerHTML=m}}function Me(e){const t=document.getElementById("res-annuel"),l=e.lignes,s=l.map(r=>r.smic),a=`
    <thead><tr>
      ...
      <th>NET</th>
      <th>COÛT EMPL.</th>
    </tr></thead>`,n=l.map((r,f)=>{const v=f>0&&r.smic!==s[f-1],$=r.mois_libelle.includes("13e"),h=parseFloat(r.fillon_regularise)-parseFloat(r.fillon_simple),I=Math.abs(h)<.005?'<span style="color:var(--dim)">—</span>':`<span class="delta-nonzero">${h>0?"+":""}${Se(h.toFixed(2))}</span>`;return`<tr class="${[v?"smic-change":"",$?"treizieme-mois":""].filter(Boolean).join(" ")}">
      <td>${r.mois_libelle}</td>
      <td>${u(r.smic)}</td>
      <td>${u(r.brut)}</td>
      <td class="c-sal">− ${u(r.total_sal)}</td>
      <td class="c-pat">+ ${u(r.total_pat_brut)}</td>
      <td class="c-alleg">− ${u(r.fillon_regularise)}</td>
      <td>${I}</td>
      <td class="c-green">${u(r.net_a_payer)}</td>
      <td class="c-yellow">${u(r.cout_employeur)}</td>
    </tr>`}).join(""),i=`
    <tr class="ann-total">
      <td>TOTAL ${e.annee}</td>
      <td></td>
      <td>${u(e.total_brut)}</td>
      <td class="c-sal">− ${u(e.total_sal)}</td>
      <td class="c-pat">+ ${u(e.total_pat_brut)}</td>
      <td class="c-alleg">− ${u(e.total_fillon)}</td>
      <td></td>
      <td class="c-green">${u(e.total_net)}</td>
      <td class="c-yellow">${u(e.total_cout)}</td>
    </tr>`,o=parseFloat(e.total_pat_brut),b=parseFloat(e.total_fillon),c=`
    <div style="display:flex;gap:1rem;flex-wrap:wrap;margin-top:0.75rem;font-size:0.72rem">
      <div style="border:1px solid var(--border);padding:0.5rem 0.9rem;background:var(--bg3)">
        ...
        <div style="color:var(--green);font-size:1.1rem;font-weight:bold">− ${u(e.total_fillon)}</div>
      </div>
      <div style="border:1px solid var(--border);padding:0.5rem 0.9rem;background:var(--bg3)">
        <div style="color:var(--muted)">TAUX FILLON MOYEN</div>
        <div style="color:var(--blue);font-size:1.1rem;font-weight:bold">
          ${o>0?(b/parseFloat(e.total_brut)*100).toFixed(2)+" %":"—"}
        </div>
      </div>
      <div style="border:1px solid var(--border);padding:0.5rem 0.9rem;background:var(--bg3)">
        <div style="color:var(--muted)">COÛT EMPLOYEUR ANNUEL</div>
        <div style="color:var(--yellow);font-size:1.1rem;font-weight:bold">${u(e.total_cout)}</div>
      </div>
    </div>`;t.innerHTML=`
    ...
    </div>`}async function Fe(e){forgeNav("detail");const t=document.getElementById("forge-profil-content");t.innerHTML='<div style="color:var(--muted);font-size:0.75rem">chargement…</div>';try{let l=C.find(s=>s.pseudo.toLowerCase()===e.toLowerCase());if(!l){const s=await fetch(`/profil/${encodeURIComponent(e)}`);if(!s.ok)throw new Error(`HTTP ${s.status} — ${await s.text()||s.statusText}`);l=await s.json()}t.innerHTML=Ne(l)}catch(l){t.innerHTML=`<div style="color:var(--red);font-size:0.75rem">Erreur : ${p(B(l))}</div>`}}function Ne(e){const t=e.linkedin_url?`<a class="profil-linkedin" href="${p(e.linkedin_url)}" target="_blank" rel="noopener noreferrer">↗ LinkedIn</a>`:"",s=[{niveau:"Maîtrisée",cls:"m",items:e.expertises.filter(i=>i.niveau==="Maîtrisée")},{niveau:"Pratiquée",cls:"p",items:e.expertises.filter(i=>i.niveau==="Pratiquée")},{niveau:"Connue",cls:"c",items:e.expertises.filter(i=>i.niveau==="Connue")}].filter(i=>i.items.length>0).map(i=>`
    <tr class="profil-ccn-section"><td colspan="3">${p(i.niveau)}</td></tr>
    ${i.items.map(o=>`
    <tr>
      <td class="profil-ccn-idcc">${p(o.ccn_idcc)}</td>
      <td>${p(o.ccn_libelle)}</td>
      <td><span class="ccn-badge ${i.cls}">${p(i.niveau)}</span></td>
    </tr>`).join("")}`).join(""),a=e.expertises.length===0?'<div style="color:var(--muted);font-size:0.72rem">Aucune CCN renseignée.</div>':`<table class="profil-ccn-tbl">${s}</table>`,n=e.created_at?G(e.created_at.slice(0,10)):"—";return`
    <div class="profil-head">
      <div>
        <div class="profil-pseudo">${p(e.pseudo)}</div>
        <div class="profil-poste">${p(e.poste)} <span style="color:var(--dim);font-size:0.85em">(${e.poste_est_actuel?"poste actuel":"poste visé"})</span></div>
        ${t}
      </div>
    ...
    <div class="profil-body">
      ...
      ${e.paie_fr_niveau?`<span class="ccn-badge ${e.paie_fr_niveau==="Maîtrisée"?"m":e.paie_fr_niveau==="Pratiquée"?"p":"c"}" style="font-size:0.75rem;padding:0.2rem 0.6rem">${p(e.paie_fr_niveau)}</span>`:'<span style="color:var(--dim);font-size:0.7rem">non renseigné</span>'}
      ...
      <table class="profil-ccn-tbl">
        ${[{niveau:"Maîtrisée",cls:"m",items:e.pays.filter(i=>i.niveau==="Maîtrisée")},{niveau:"Pratiquée",cls:"p",items:e.pays.filter(i=>i.niveau==="Pratiquée")},{niveau:"Connue",cls:"c",items:e.pays.filter(i=>i.niveau==="Connue")}].filter(i=>i.items.length>0).map(i=>`
            ...
            ${i.items.map(o=>`
            <tr>
              <td class="profil-ccn-idcc">${p(o.pays_code)}</td>
              <td>${p(o.pays_libelle)}</td>
              <td><span class="ccn-badge ${i.cls}">${p(i.niveau)}</span></td>
            </tr>`).join("")}`).join("")}
      </table>`:""}
    ...
    <button type="button" class="forge-ccn-del" onclick="forgeSupprPays(${e})" title="Supprimer">×</button>`,document.getElementById("forge-pays-list").appendChild(t)};window.forgeSupprPays=function(e){document.getElementById("forge-pays-"+e)?.remove()};let be=0;window.forgeAjouterCcn=function(){const e=++be,t=document.createElement("div");t.className="forge-ccn-row",t.id="forge-ccn-"+e,t.innerHTML=`
    <select class="forge-ccn-select">${Be}</select>
    <select class="forge-ccn-niveau">
      <option value="Connue">Connue</option>
      <option value="Pratiquée">Pratiquée</option>
      <option value="Maîtrisée" selected>Maîtrisée</option>
    </select>
    <button type="button" class="forge-ccn-del" onclick="forgeSupprCcn(${e})" title="Supprimer">×</button>`,document.getElementById("forge-ccn-list").appendChild(t)};window.forgeSupprCcn=function(e){document.getElementById("forge-ccn-"+e)?.remove()};window.forgeSoumettre=async function(e){e.preventDefault();const t=document.getElementById("forge-form"),l=document.getElementById("forge-form-err"),s=document.getElementById("forge-submit-btn");l.textContent="";const a=[];document.querySelectorAll('[id^="forge-pays-"]').forEach(o=>{const b=o.querySelector(".forge-pays-select")?.value,c=o.querySelector(".forge-ccn-niveau")?.value,r=me.find(f=>f.code===b);b&&r&&a.push({pays_code:b,pays_libelle:r.libelle,niveau:c})});const n=[];document.querySelectorAll('.forge-ccn-row:not([id^="forge-pays-"])').forEach(o=>{const b=o.querySelector(".forge-ccn-select").value,c=o.querySelector(".forge-ccn-niveau").value,r=ue.find(f=>f.idcc===b);b&&r&&n.push({ccn_idcc:b,ccn_libelle:r.libelle,niveau:c})});const i={email:t.querySelector('[name="email"]').value.trim(),pseudo:t.querySelector('[name="pseudo"]').value.trim(),poste:t.querySelector('[name="poste"]').value.trim(),linkedin_url:t.querySelector('[name="linkedin_url"]').value.trim()||null,poste_est_actuel:t.querySelector('[name="poste_est_actuel"]').value!=="0",paie_fr_niveau:t.querySelector('[name="paie_fr_niveau"]').value||null,pays:a,expertises:n};if(!i.email){l.textContent="Email requis.";return}if(!i.pseudo){l.textContent="Pseudo requis.";return}if(!i.poste){l.textContent="Poste requis.";return}s.disabled=!0,s.textContent="[ envoi… ]";try{const o=await fetch("/forge/profil",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(i)});if(!o.ok)throw new Error(`HTTP ${o.status} — ${await o.text()||o.statusText}`);const b=await o.json();C.unshift(b),t.reset(),document.getElementById("forge-pays-list").innerHTML="",document.getElementById("forge-ccn-list").innerHTML="",pe=0,be=0,Fe(b.pseudo)}catch(o){l.textContent=B(o),s.disabled=!1,s.textContent="[ Rejoindre la Forge ]"}};const qe=[{prenom:"Geralt",nom:"de Riv"},{prenom:"Sam",nom:"Vimes"},{prenom:"Elric",nom:"de Melniboné"},{prenom:"Druss",nom:"la Légende"},{prenom:"Logen",nom:"Neuf-Doigts"},{prenom:"Aragorn",nom:"Grands-Pas"},{prenom:"Jon",nom:"Shannow"},{prenom:"Salim",nom:"Dhibi"},{prenom:"Bayaz",nom:"le Magi"},{prenom:"Merlin",nom:"l'Enchanteur"}],ke=[{prenom:"Lyra",nom:"Belacqua"},{prenom:"Hermione",nom:"Granger"},{prenom:"Eowyn",nom:"du Rohan"},{prenom:"Ellana",nom:"Caldin"},{prenom:"Ferro",nom:"Maljinn"},{prenom:"Magrat",nom:"Garlick"},{prenom:"Ewilan",nom:"Gil'Sayan"},{prenom:"Sigarni",nom:"la Guerrière"},{prenom:"Rikke",nom:"la Nord"},{prenom:"Tanaquil",nom:"la Magicienne"}],se=[17,16,16,15,15,15,14,14,14,13,13,11],M=se[Math.floor(Math.random()*se.length)]/100;let ie="H",J=!1;function ae(e){return e[Math.floor(Math.random()*e.length)]}function fe(e,t){["d-prenom","m-prenom"].forEach(l=>{const s=document.getElementById(l);s&&(s.value=e)}),["d-nom","m-nom"].forEach(l=>{const s=document.getElementById(l);s&&(s.value=t)}),J=!1}function ge(e,t=!1){const l=e==="H";["d-hf-h","m-hf-h"].forEach(s=>{document.getElementById(s)?.classList.toggle("ptog-on",l),document.getElementById(s)?.classList.toggle("ptog-off",!l)}),["d-hf-f","m-hf-f"].forEach(s=>{document.getElementById(s)?.classList.toggle("ptog-on",!l),document.getElementById(s)?.classList.toggle("ptog-off",l)}),t&&document.querySelectorAll(".genre-ecart-hint").forEach(s=>{s.textContent=l?s.dataset.textHf:s.dataset.textFh,s.style.display="inline"})}window.setGenre=function(e){if(e===ie)return;if(!J){const l=e==="F"?window._heroF:window._heroH;fe(l.prenom,l.nom)}const t=e==="F"?1-M:1/(1-M);["d-brut","m-brut"].forEach(l=>{const s=document.getElementById(l);s&&(s.value=Math.round(parseFloat(s.value)*t))}),ie=e,ge(e,!0)};const X=document.getElementById("burger-btn"),O=document.getElementById("burger-menu");function Oe(){X.classList.add("open"),O.classList.add("open")}window.closeBurger=function(){X.classList.remove("open"),O.classList.remove("open")};X.addEventListener("click",e=>{e.stopPropagation(),O.classList.contains("open")?closeBurger():Oe()});document.addEventListener("click",()=>closeBurger());O.addEventListener("click",e=>e.stopPropagation());
