...
`);r.forEach((h,C)=>i.set(h,g[C]??h))}l.forEach(a=>{const n=G.get(a);a.isConnected&&i.has(n)&&(a.textContent=i.get(n))}),document.documentElement.lang=e,Ee=e}catch(a){console.error("Traduction échouée :",a),t.textContent="🌐 ✗",setTimeout(()=>{t.textContent="🌐 LANGUE",t.classList.remove("loading")},2e3);return}t.textContent="🌐 LANGUE",t.classList.remove("loading")};document.addEventListener("click",e=>{!e.target.closest("#trad-btn")&&!e.target.closest("#trad-panel")&&(document.getElementById("trad-panel")?.classList.remove("open"),document.getElementById("trad-btn")?.classList.remove("open"))});window.toggleA11yPanel=function(){const e=document.getElementById("a11y-panel"),t=document.getElementById("a11y-btn"),l=e.classList.toggle("open");t.classList.toggle("open",l)};window.toggleHVMode=function(){const e=document.body.classList.toggle("hv-mode");document.getElementById("hv-switch")?.classList.toggle("on",e),document.getElementById("a11y-hv-btn")?.classList.toggle("active",e),localStorage.setItem("xenna-hv",e?"1":"")};window.toggleZoom=function(){const e=document.body.classList.toggle("zoom-mode");document.documentElement.style.zoom=e?"200%":"",document.getElementById("zoom-switch")?.classList.toggle("on",e),document.getElementById("a11y-magnifier")?.classList.toggle("active",e),localStorage.setItem("xenna-zoom",e?"1":"")};const xe=new Set,lt=new Set(["IBM Plex Mono","Fira Code","JetBrains Mono","Source Code Pro","Roboto Mono","Inconsolata"]);window.setAppFont=function(e,t=!1){if(!e){document.body.classList.remove("custom-font"),document.documentElement.style.removeProperty("--app-font"),localStorage.removeItem("xenna-font");const i=document.getElementById("font-picker");i&&(i.value="");return}if(!lt.has(e))return;const l=e.replace(/ /g,"+");if(!xe.has(l)){const i=document.createElement("link");i.rel="stylesheet",i.href=`https://fonts.googleapis.com/css2?family=${l}&display=swap`,document.head.appendChild(i),xe.add(l)}document.documentElement.style.setProperty("--app-font",`'${e}', monospace`),document.body.classList.add("custom-font"),localStorage.setItem("xenna-font",e);const s=document.getElementById("font-picker");s&&t&&(s.value=e)};const R=[];window.scan67=function(){const e=Date.now();for(R.push(e);R.length&&e-R[0]>1500;)R.shift();const t=R.length>=3;t&&(R.length=0);const l=t?/42/:/67/,i=Array.from(document.querySelectorAll(".mob-val, .sb-val, .ascii-tbl td, .fm-val, .fm-result td")).filter(a=>l.test(a.textContent.replace(/[\s ]/g,""))&&a.offsetParent!==null);if(i.length===0)return;const o=document.getElementById("a11y-67-btn");if(o.classList.add("active"),t){const a=["#ff0055","#ff6600","#ffcc00","#00ff88","#00ccff","#aa00ff","#ff00cc","#39ff14","#ff4444","#44ffff","#ff69b4","#7fff00"];i.forEach((n,r)=>{setTimeout(()=>{const f=a[Math.floor(Math.random()*a.length)];Object.assign(n.style,{background:f,color:"#000",outline:`2px solid ${f}`,borderRadius:"2px",transition:"all 0.15s"}),setTimeout(()=>Object.assign(n.style,{background:"",color:"",outline:"",borderRadius:""}),900)},r*250)}),setTimeout(()=>o.classList.remove("active"),i.length*250+1e3)}else i.forEach((a,n)=>{setTimeout(()=>{a.classList.remove("flash-67"),a.offsetWidth,a.classList.add("flash-67"),a.addEventListener("animationend",()=>a.classList.remove("flash-67"),{once:!0})},n*500)}),setTimeout(()=>o.classList.remove("active"),i.length*500+200)};window.toggleBWMode=function(){const e=document.body.classList.toggle("bw-mode");document.getElementById("bw-switch")?.classList.toggle("on",e),document.getElementById("a11y-bw-btn")?.classList.toggle("active",e),localStorage.setItem("xenna-bw",e?"1":"")};let k=!1,Ie=0;const M=e=>new Promise(t=>setTimeout(t,e));window.toggleDactylo=function(){k=!k,document.getElementById("dactylo-switch")?.classList.toggle("on",k),localStorage.setItem("xenna-dactylo",k?"1":"")};async function st(e){const t=++Ie,l=()=>t!==Ie,s=document.getElementById("res-desktop"),i=s.querySelectorAll("tr.data-row");if(!i.length)return;const a=((e?.salarie?.prenom||"")+(e?.salarie?.nom||"")).toLowerCase().includes("ë"),n=a?2:4,r=a?1:2,f=["#ff00ff","#00ffff","#ff0066","#66ff00","#ff6600","#0066ff","#ff00cc","#00ff99","#ffff00","#ff3399"],p=()=>a?f[Math.floor(Math.random()*f.length)]:"#ffe066",E=s.querySelector("tr.tbl-total");let c=null,g=null,h="",C="";if(E){const I=E.querySelectorAll("td");c=I[1],g=I[3],c&&(h=c.textContent,c.textContent=""),g&&(C=g.textContent,g.textContent="")}const $=[];for(const I of i){const y=I.querySelector("td:first-child > span:last-child");if(y){const w=y.textContent;y.textContent="",$.push({target:y,text:w,ms:n})}I.querySelectorAll("td:not(:first-child)").forEach(w=>{const S=[...w.childNodes].find(x=>x.nodeType===3&&x.textContent.trim());if(S){const x=S.textContent;S.textContent="",$.push({target:S,text:x,ms:r})}}),$.push({pause:a?4:8})}for(const I of $){if(l())return;if(I.pause){await M(I.pause);continue}for(const y of I.text){if(l())return;I.target.textContent+=y,await M(I.ms)}}if(!l()){await M(80);for(const I of i){if(l())return;const y=I.querySelectorAll("td")[3];if(y?.classList.contains("c-sal")){const w=p();y.style.background=w,y.style.color="#000",await M(a?65:110),y.style.background="",y.style.color="",await M(a?10:20)}}if(c){a&&(c.style.fontWeight="bold",c.style.fontSize="1.05em");for(const I of h){if(l())return;c.textContent+=I,await M(r)}a&&Ce(c)}if(!l()){await M(80);for(const I of i){if(l())return;const y=I.querySelectorAll("td")[5];if(y?.classList.contains("c-pat")){const w=p();y.style.background=w,y.style.color="#000",await M(a?65:110),y.style.background="",y.style.color="",await M(a?10:20)}}if(g){a&&(g.style.fontWeight="bold",g.style.fontSize="1.05em");for(const I of C){if(l())return;g.textContent+=I,await M(r)}a&&Ce(g)}}}}let Se=!1;function at(){if(Se)return;Se=!0;const e=document.createElement("style");e.textContent="@keyframes flameRise{0%{transform:translateY(0) scale(1);opacity:1}100%{transform:translateY(-42px) scale(0);opacity:0}}",document.head.appendChild(e)}function Ce(e){at();const t=e.getBoundingClientRect(),l=document.createElement("div");l.style.cssText=`position:fixed;pointer-events:none;z-index:9999;overflow:visible;left:${t.left}px;top:${t.top}px;width:${t.width}px;height:${t.height}px`,document.body.appendChild(l);const s=["#ff8800","#ffdd00","#ff5500","#ffaa00","#ff3300","#ffcc00","#ff6600"],i=setInterval(()=>{const o=document.createElement("div"),a=Math.floor(Math.random()*3)+1,n=(.5+Math.random()*1.2).toFixed(2);o.style.cssText=`position:absolute;pointer-events:none;left:${(Math.random()*t.width).toFixed(1)}px;bottom:0;width:${a}px;height:${a}px;background:${s[Math.floor(Math.random()*s.length)]};animation:flameRise ${n}s ease-out forwards`,l.appendChild(o),setTimeout(()=>o.remove(),parseFloat(n)*1e3+50)},35);setTimeout(()=>{clearInterval(i),setTimeout(()=>l.remove(),1400)},3e3)}function u(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}window.setView=function(e){["mobile","desktop","annuel","forge","apropos","contact","gaabrielle","hercule","quizz"].forEach(t=>document.body.classList.toggle("is-"+t,e===t)),document.getElementById("btn-desk").classList.toggle("active",e==="desktop"),document.getElementById("btn-mob").classList.toggle("active",e==="mobile"),document.getElementById("btn-ann").classList.toggle("active",e==="annuel"),de&&(e==="desktop"||e==="mobile")&&ze(de),e==="forge"&&pt(),e==="quizz"&&Lt()};let X="EUR";function d(e){const t=parseFloat(e),l=X==="CHF"?" CHF":" €";return t.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+l}function it(e,t=!1){const l=parseFloat(e),s=X==="CHF"?" CHF":" €",i=l.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+s;return t&&l>0?"+"+i:i}function se(e){return(parseFloat(e)*100).toFixed(2)+" %"}function Pe(){const e=document.body.classList.contains("is-mobile")?"m-date":"d-date",t=e==="m-date"?"d-date":"m-date";return document.getElementById(e)?.value||document.getElementById(t)?.value||Fe}function be(e){if(!e)return"—";const[t,l,s]=e.split("-");return`${s}/${l}/${t}`}const nt=[{min:0,max:1620,taux:0},{min:1620,max:1683,taux:.005},{min:1683,max:1791,taux:.013},{min:1791,max:1911,taux:.021},{min:1911,max:2042,taux:.029},{min:2042,max:2151,taux:.035},{min:2151,max:2294,taux:.041},{min:2294,max:2714,taux:.053},{min:2714,max:3107,taux:.075},{min:3107,max:3539,taux:.099},{min:3539,max:3983,taux:.119},{min:3983,max:4648,taux:.138},{min:4648,max:5574,taux:.158},{min:5574,max:6974,taux:.179},{min:6974,max:8711,taux:.2},{min:8711,max:12091,taux:.24},{min:12091,max:16376,taux:.28},{min:16376,max:25706,taux:.33},{min:25706,max:55062,taux:.38},{min:55062,max:1/0,taux:.43}];function ge(e){const t=parseFloat(e);if(isNaN(t)||t<=0)return{total:0,taux_effectif:0,details:[]};let l=0;const s=[];for(const i of nt){if(t<=i.min)break;const a=+((i.max===1/0?t:Math.min(t,i.max))-i.min).toFixed(2),n=a*i.taux;if(s.push({min:i.min,max:i.max===1/0?null:i.max,taux:i.taux,base:a,montant:+n.toFixed(2)}),l+=n,i.max===1/0||t<=i.max)break}return{total:+l.toFixed(2),taux_effectif:t>0?l/t:0,details:s}}const $e={"Sécurité Sociale":"cat-ss","CSG/CRDS":"cat-csg","Retraite complémentaire":"cat-ret",Prévoyance:"cat-prev",Chômage:"cat-cho",Allègement:"cat-alleg","1er pilier":"cat-ss","Assurance chômage":"cat-cho","Assurance accidents":"cat-acc","Prévoyance maladie":"cat-prev","Prévoyance (LPP)":"cat-ret","Assurance pension":"cat-ret","Assurance maladie":"cat-ss","Assurance dépendance":"cat-prev","Mutualité des employeurs":"cat-ss","Previdenza sociale":"cat-ss",Disoccupazione:"cat-cho","Assicurazione infortuni":"cat-acc","Fine rapporto":"cat-prev",Allegement:"cat-alleg","Bonus IRPEF":"cat-alleg",Imposta:"cat-csg","Imposta regionale":"cat-csg","Retraite fédérale":"cat-ret","Retraite Québec":"cat-ret","Chômage fédéral":"cat-cho","Parentalité Québec":"cat-ss","Santé Québec":"cat-ss","Impôt fédéral":"cat-csg","Impôt provincial":"cat-csg",Autres:"cat-prev"},z={},we={SS_VIEILLESSE_PLAF:"min(Salaire brut, Plafond Mensuel Sécurité Sociale — PMSS)",CHOMAGE:"min(Salaire brut, 4 × PMSS)",AGS:"min(Salaire brut, 4 × PMSS)",CSG_DEDUCTIBLE:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels (CSS art. L136-2)",CSG_NON_DEDUCTIBLE:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels",CRDS:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels",AGIRC_ARRCO_T1:"min(Salaire brut, PMSS)  — Tranche 1 (entre 0 et 1 PMSS)",AGIRC_ARRCO_CEG_T1:"min(Salaire brut, PMSS)  — Tranche 1",PREVOYANCE_CADRE_MIN:"min(Salaire brut, PMSS)  — Tranche A",AGIRC_ARRCO_T2:"Fraction du salaire entre 1 PMSS et 8 PMSS  — Tranche 2",AGIRC_ARRCO_CEG_T2:"Fraction du salaire entre 1 PMSS et 8 PMSS  — Tranche 2"};function Q(e,t=!0){const l=["f","(","x",")"].map((s,i)=>`<span style="animation-delay:${i*45}ms">${s}</span>`).join("");return t?`<span class="formula-star" data-fmkey="${e}" onclick="event.stopPropagation();showFormula('${e}')">${l}</span>`:`<span class="formula-star" aria-hidden="true">${l}</span>`}window.togglePasDetail=function(e){const t=document.getElementById(e);if(!t)return;const l=t.style.display!=="none";t.style.display=l?"none":"block";const s=document.getElementById(e+"-arrow");s&&(s.textContent=l?"▶":"▼")};function V(e,t){if(e.code==="REDUCTION_FILLON")return`<pre class="fm-fillon">${u(e.explication)}</pre>`;const l=t==="sal",s=l?e.taux_sal:e.taux_pat,i=parseFloat(e.base),o=l?parseFloat(e.montant_sal):Math.abs(parseFloat(e.montant_pat)),a=l?"Taux salarial":"Taux patronal",n=l?"Montant salarial":t==="alleg"?"Montant allègement":"Montant patronal",r=l?"c-sal":t==="alleg"?"c-alleg":"c-pat",f=we[e.code]?`<div class="fm-base-note">Assiette  =  ${u(we[e.code])}</div>`:"";return`
    ...
    <table class="pas-tbl">
      ...
      <tr class="${a?"pas-zero":""}">
        ...
        <td class="r ${a?"c-dim":"c-purple"}">${a?"—":d(s.montant)}</td>
      </tr>`}).join("")}</tbody>
      <tfoot>
        <tr>
          <td>Net imposable</td>
    ...
    <div class="summary-bar">
      ...
      <div class="sb-cell">
        ...
        <div class="sb-ded">
          <div class="sb-ded-row">
            ...
            <span style="color:var(--red)">− ${d(i?y:I)}</span>
          </div>
          ${f?`<div class="sb-ded-row">
            <span>Impôt à la source (${(E*100).toFixed(1)} %)</span>
            <span class="fm-val" style="color:var(--purple);cursor:pointer" onclick="showFormula('CH_IS')">− ${d(p)}${Q("CH_IS")}</span>
          </div>`:""}
          ${c?`<div class="sb-ded-row">
            <span>IRPEF (${(h*100).toFixed(1)} % eff.)</span>
            <span style="color:var(--purple)">− ${d(g)}</span>
          </div>`:""}
          ${C?`<div class="sb-ded-row">
            <span>Bonus cuneo fiscale</span>
            <span style="color:var(--green)">+ ${d(Math.abs($))}</span>
          </div>`:""}
          ${s?"":`<div class="sb-ded-row">
            <span>PAS (${(n.taux_effectif*100).toFixed(1)} %)</span>
            <span class="fm-val" style="color:var(--purple);cursor:pointer" onclick="showFormula('PAS')">− ${d(n.total)}${Q("PAS")}</span>
          </div>`}
      ...
      <div class="sb-cell">
        ...
        <div class="sb-val c-green">${d(r)}</div>
      </div>
      <div class="sb-cell">
        <div class="sb-lbl">▸ CHARGES PAT.</div>
        <div class="sb-val c-orange">${d(a)}</div>
      </div>
      <div class="sb-cell">
        <div class="sb-lbl">▸ SUPER BRUT</div>
        <div class="sb-val c-yellow">${d(parseFloat(e.brut)+a)}</div>
      </div>
    </div>`,S=l.filter(b=>b.categorie!=="Allègement"&&(parseFloat(b.montant_sal)>0||b.taux_sal!=="0"||parseFloat(b.montant_pat)>0)),x=l.filter(b=>b.categorie==="Allègement"),F=S.reduce((b,_)=>b+parseFloat(_.montant_pat),0);function ie(b,_){return b.map((v,P)=>{const B=_+P,A=$e[v.categorie]||"cat-ss",oe=parseFloat(v.montant_sal)>0?"c-sal":"c-dim",ce=parseFloat(v.montant_pat)>0?"c-pat":"c-dim",U=`${v.code}_sal`,j=`${v.code}_pat`,te=parseFloat(v.montant_sal)>0,le=parseFloat(v.montant_pat)>0;te&&(z[U]={c:v,type:"sal"}),le&&(z[j]={c:v,type:"pat"});const Xe=Q(U,te),We=Q(j,le);return`
        <tr class="data-row" id="row-${B}" onclick="toggleExpl(${B})">
          <td>
            <span class="expand-icon">▶</span>
            <span class="cat ${A}">[${v.categorie}]</span>
            <span>${v.libelle}</span>
          </td>
          <td class="r">${d(v.base)}</td>
          <td class="r">${parseFloat(v.taux_sal)>0?"− ":""}${se(v.taux_sal)}</td>
          <td class="r ${oe}"${te?` onclick="event.stopPropagation();showFormula('${U}')" style="cursor:pointer"`:""}>${te?"− ":""}${d(v.montant_sal)}${Xe}</td>
          <td class="r">${parseFloat(v.taux_pat)>0?"− ":""}${se(v.taux_pat)}</td>
          <td class="r ${ce}"${le?` onclick="event.stopPropagation();showFormula('${j}')" style="cursor:pointer"`:""}>${le?"− ":""}${d(v.montant_pat)}${We}</td>
        </tr>
        <tr class="expl-row" id="expl-${B}" style="display:none">
          <td colspan="6">
            <div class="expl-box">
              <div class="expl-txt">▸ ${u(v.explication)}</div>
              ${v.loi_ref?`<div class="expl-ref">§ ${u(v.loi_ref)}</div>`:""}
            </div>
          </td>
        </tr>`}).join("")}const Z=`
    <colgroup>
  ...
  </div>`,m=x.reduce((b,_)=>b+parseFloat(_.montant_pat),0),L=x.length===0?"":`
    <div class="tbl-section-head">── ALLÈGEMENTS PATRONAUX ───────────────────────────────────────────</div>
    <table class="ascii-tbl">
      ${Z}
      <tbody>
        ${x.map((b,_)=>{const v=S.length+_,P=$e[b.categorie]||"cat-alleg",B=Math.abs(parseFloat(b.montant_pat)),A=`${b.code}_alleg`;return z[A]={c:b,type:"alleg"},`
            <tr class="data-row" id="row-${v}" onclick="toggleExpl(${v})">
              <td>
                <span class="expand-icon">▶</span>
                <span class="cat ${P}">[${b.categorie}]</span>
                <span>${b.libelle}</span>
              </td>
              <td class="r">${d(b.base)}</td>
              <td class="r"></td>
              <td class="r"></td>
              <td class="r c-alleg">${se(Math.abs(parseFloat(b.taux_pat)))}</td>
              <td class="r c-alleg" onclick="event.stopPropagation();showFormula('${A}')" style="cursor:pointer">− ${d(B)}${Q(A)}</td>
            </tr>
            <tr class="expl-row" id="expl-${v}" style="display:none">
              <td colspan="6">
                <div class="expl-box">
                  <div class="expl-txt">▸ ${u(b.explication)}</div>
                  ${b.loi_ref?`<div class="expl-ref">§ ${u(b.loi_ref)}</div>`:""}
                </div>
              </td>
            </tr>`}).join("")}
        <tr class="tbl-total">
          <td colspan="5">TOTAL ALLÈGEMENTS PATRONAUX</td>
          <td class="r c-alleg">− ${d(Math.abs(m))}</td>
        </tr>
      </tbody>
    </table>`;t.innerHTML=ee+w+`<div class="tbl-wrap">${ne}${L}</div>`}window.mobToggle=function(e,t){const l=document.getElementById("mob-expand-"+e);if(!l)return;const s=["why","how","sal","pat"],i=l.style.display!=="none",o=l.dataset.panel,a=n=>{const r=document.getElementById(`mob-expand-${e}-${n}`);r&&(r.style.display=n===t?"block":"none")};i?o===t?l.style.display="none":(l.dataset.panel=t,s.forEach(a)):(l.style.display="block",l.dataset.panel=t,s.forEach(a))};function ct(e,t,l,s,i,o=0){const a=e.code==="REDUCTION_FILLON"?`<pre class="fm-fillon">${u(e.explication)}</pre>`:`<div class="fm-type-${i}">${V(e,i)}</div>`,n=`
    ...
    </div>`}function rt(e){const t=document.getElementById("res-mobile"),l=document.getElementById("m-nom")?.value||document.getElementById("d-nom")?.value||"",s=document.getElementById("m-prenom")?.value||document.getElementById("d-prenom")?.value||"",i=e.cotisations,o=["suisse","luxembourg","italia","canada","quebec"].includes(e.salarie?.pays),a=e.salarie?.pays==="italia",n=i.reduce((m,L)=>m+parseFloat(L.montant_sal),0),r=i.reduce((m,L)=>m+parseFloat(L.montant_pat),0),f=o?{total:0,taux_effectif:0}:ge(e.net_imposable),p=parseFloat(e.net_a_payer)-f.total,E=parseFloat(e.brut)+r,c=e.salarie?.pays==="suisse"?i.find(m=>m.code==="CH_IS"):null,g=c?parseFloat(c.montant_sal):0,h=c?parseFloat(c.taux_sal):0,C=a?i.find(m=>m.code==="IT_IRPEF"):null,$=C?parseFloat(C.montant_sal):0,I=C?parseFloat(C.taux_sal):0,y=a?i.find(m=>m.code==="IT_BONUS_CUNEO"):null,w=y?parseFloat(y.montant_sal):0,S=n-g-$-w,x=i.filter(m=>m.categorie!=="Allègement"&&m.code!=="CH_IS"&&m.code!=="IT_IRPEF"&&m.code!=="IT_BONUS_CUNEO"&&(parseFloat(m.montant_sal)>0||m.taux_sal!=="0"||parseFloat(m.montant_pat)>0)),F=i.filter(m=>m.categorie==="Allègement"),ie=x.reduce((m,L)=>m+parseFloat(L.montant_pat),0),Z=F.reduce((m,L)=>m+parseFloat(L.montant_pat),0),ne=x.map((m,L)=>{const b=parseFloat(m.montant_sal)>0,_=parseFloat(m.montant_pat)>0,v=`${m.code}_u`,P=m.code==="REDUCTION_FILLON",B=b?P?`<pre class="fm-fillon">${u(m.explication)}</pre>`:`<div class="fm-type-sal">${V(m,"sal")}</div>`:"",A=_?P?`<pre class="fm-fillon">${u(m.explication)}</pre>`:`<div class="fm-type-pat">${V(m,"pat")}</div>`:"",oe=`
      <div class="mob-exp-txt">${u(m.explication)}</div>
      ${m.loi_ref?`<div class="mob-exp-loi">§ ${u(m.loi_ref)}</div>`:""}`,ce=`mob-stripe-sal-${L%2===0?"a":"b"}`,U=b?`<span class="mob-val mob-cot-amt" style="color:#ffe033" onclick="mobToggle('${v}','sal')">− ${d(m.montant_sal)}</span>`:`<span class="mob-val c-dim">0 ${X==="CHF"?"CHF":"€"}</span>`,j=_?`<span class="mob-val c-orange mob-cot-amt" onclick="mobToggle('${v}','pat')">− ${d(m.montant_pat)}</span>`:`<span class="mob-val c-dim">0 ${X==="CHF"?"CHF":"€"}</span>`;return`
      <div class="${ce}">
        <div class="mob-row">
          <span class="mob-lbl mob-cot-lbl" onclick="mobToggle('${v}','why')">${u(m.libelle)}</span>
          <span style="display:flex;flex-direction:column;align-items:flex-end;gap:0.1rem">${U}${j}</span>
        </div>
        <div class="mob-expand" id="mob-expand-${v}" style="display:none">
          <div id="mob-expand-${v}-why">${oe}</div>
          ${B?`<div id="mob-expand-${v}-sal" style="display:none">${B}</div>`:""}
          ${A?`<div id="mob-expand-${v}-pat" style="display:none">${A}</div>`:""}
        </div>
      </div>`}).join(""),ee=F.map((m,L)=>ct(m,`${m.code}_alleg`,`− ${d(Math.abs(parseFloat(m.montant_pat)))}`,"c-alleg","alleg",L)).join("");t.innerHTML=`
    <div class="mob-bulletin">
      ...
      <div class="mob-row subtot">
        ...
        <span class="mob-val c-orange">− ${d(ie)}</span>
      </div>
      ...
      <div id="is-detail-mob" style="display:none;padding:0.4rem 0.6rem 0.2rem">
        <div class="fm-type-sal">${V(c,"sal")}</div>
        <div class="mob-exp-txt" style="margin-top:0.5rem">${u(c.explication)}</div>
        ${c.loi_ref?`<div class="mob-exp-loi">§ ${u(c.loi_ref)}</div>`:""}
      </div>`:""}
      ...
      ${o?"":`<div class="mob-row pas-row" style="cursor:pointer" onclick="togglePasDetail('pas-detail-mob')">
        ...
        <span class="mob-val c-purple">− ${d(f.total)}</span>
      </div>
      ...
      <!-- IRPEF italienne -->
      ${C?`<div class="mob-row pas-row" style="cursor:pointer" onclick="togglePasDetail('irpef-detail-mob')">
        <span class="mob-lbl">IRPEF (${(I*100).toFixed(1)} % eff.) <span id="irpef-detail-mob-arrow" style="font-size:0.65em">▶</span></span>
        <span class="mob-val c-purple">− ${d($)}</span>
      </div>
      <div id="irpef-detail-mob" style="display:none;padding:0.4rem 0.6rem 0.2rem">
        <div class="mob-exp-txt">${u(C.explication)}</div>
        ${C.loi_ref?`<div class="mob-exp-loi">§ ${u(C.loi_ref)}</div>`:""}
      </div>`:""}

      <!-- Bonus cuneo fiscale -->
      ${y?`<div class="mob-row" style="cursor:pointer" onclick="togglePasDetail('bonus-cuneo-mob')">
        <span class="mob-lbl">Bonus cuneo fiscale <span id="bonus-cuneo-mob-arrow" style="font-size:0.65em">▶</span></span>
        <span class="mob-val c-green">+ ${d(Math.abs(w))}</span>
      </div>
      ...
      <div class="mob-row final-row">
        ...
        <span class="mob-val c-green">${d(p)}</span>
      </div>
      ...
      <div class="mob-row superbrut">
        ...
        <span class="mob-val c-blue">${d(E)}</span>
      </div>

    </div>`}function ze(e){X=e.devise||"EUR",ot(e),rt(e),k&&st(e)}function _e(e){const t=`<div style="padding:1.5rem;color:#f87171;font-size:0.8rem">⚠ ${u(e)}</div>`;document.getElementById("res-desktop").innerHTML=t,document.getElementById("res-mobile").innerHTML=t}async function ke(e){const t=e==="mobile",l=document.getElementById(t?"m-brut":"d-brut").value,s=document.getElementById(t?"m-statut":"d-statut").value,i=document.getElementById(t?"m-nom":"d-nom").value||"Dupont",o=document.getElementById(t?"m-prenom":"d-prenom").value||"Marie",a=document.getElementById(t?"m-date":"d-date").value||Fe,n=document.getElementById(t?"m-alsace-moselle":"d-alsace-moselle")?.checked??!1,r=document.getElementById(t?"m-suisse":"d-suisse")?.checked??!1,f=document.getElementById(t?"m-luxembourg":"d-luxembourg")?.checked??!1,p=document.getElementById(t?"m-fpt":"d-fpt")?.checked??!1,E=document.getElementById(t?"m-italie":"d-italie")?.checked??!1,c=document.getElementById(t?"m-canada":"d-canada")?.checked??!1,g=document.getElementById(t?"m-quebec":"d-quebec")?.checked??!1,h=document.getElementById(t?"m-assujetti-is":"d-assujetti-is")?.checked??!1,C=document.getElementById(t?"m-canton":"d-canton")?.value||null,$=document.getElementById(t?"m-tarif-is":"d-tarif-is")?.value||null,I=parseFloat(l);if(!l||isNaN(I)||I<=0){_e("Salaire brut invalide — saisir un montant positif.");return}if(!/^\d{4}-\d{2}-\d{2}$/.test(a)){_e(`Date invalide : '${a}' (format attendu : YYYY-MM-DD).`);return}["d-brut","m-brut"].forEach(S=>{const x=document.getElementById(S);x&&(x.value=l)}),["d-statut","m-statut"].forEach(S=>{const x=document.getElementById(S);x&&(x.value=s)}),["d-nom","m-nom"].forEach(S=>{const x=document.getElementById(S);x&&(x.value=i)}),["d-prenom","m-prenom"].forEach(S=>{const x=document.getElementById(S);x&&(x.value=o)}),["d-date","m-date"].forEach(S=>{const x=document.getElementById(S);x&&(x.value=a)});const y=r?"suisse":f?"luxembourg":E?"italia":c?"canada":g?"quebec":null,w=y?"2026-01-01":a;try{const S=await Te("calculer_bulletin",{salarie:{nom:i,prenom:o,salaire_brut:l.toString(),statut:s,alsace_moselle:n,pays:y??(p?"fonction_publique":"france"),assujetti_is:h,canton:r&&h&&C?C:null,tarif_is:r&&h&&$?$:null,regione:null,contratto_termine:!1},datePaie:w});de=S,ze(S)}catch(S){console.error("[calculer_bulletin] erreur brute :",S);const x=K(S),F=`<div style="padding:1.5rem;color:#f87171;font-size:0.8rem">ERREUR : ${u(x)}</div>`;document.getElementById("res-desktop").innerHTML=F,document.getElementById("res-mobile").innerHTML=F}}function dt(e){const t=document.getElementById("res-annuel"),l=e.lignes,s=l.map(p=>p.smic),i=`
    <thead><tr>
      ...
      <th>NET</th>
      <th>COÛT EMPL.</th>
    </tr></thead>`,o=l.map((p,E)=>{const c=E>0&&p.smic!==s[E-1],g=p.mois_libelle.includes("13e"),h=parseFloat(p.fillon_regularise)-parseFloat(p.fillon_simple),C=Math.abs(h)<.005?'<span style="color:var(--dim)">—</span>':`<span class="delta-nonzero">${h>0?"+":""}${it(h.toFixed(2))}</span>`;return`<tr class="${[c?"smic-change":"",g?"treizieme-mois":""].filter(Boolean).join(" ")}">
    ...
    </tr>`,n=parseFloat(e.total_pat_brut),r=parseFloat(e.total_fillon),f=`
    <div style="display:flex;gap:1rem;flex-wrap:wrap;margin-top:0.75rem;font-size:0.72rem">
      ...
      <div style="border:1px solid var(--border);padding:0.5rem 0.9rem;background:var(--bg3)">
        ...
        <div style="color:var(--blue);font-size:1.1rem;font-weight:bold">
          ${n>0?(r/parseFloat(e.total_brut)*100).toFixed(2)+" %":"—"}
        </div>
      </div>
      <div style="border:1px solid var(--border);padding:0.5rem 0.9rem;background:var(--bg3)">
    ...
    </div>`}async function bt(e){forgeNav("detail");const t=document.getElementById("forge-profil-content");t.innerHTML='<div style="color:var(--muted);font-size:0.75rem">chargement…</div>';try{let l=N.find(s=>s.pseudo.toLowerCase()===e.toLowerCase());if(!l){const s=await fetch(`/profil/${encodeURIComponent(e)}`);if(!s.ok)throw new Error(`HTTP ${s.status} — ${await s.text()||s.statusText}`);l=await s.json()}t.innerHTML=gt(l)}catch(l){t.innerHTML=`<div style="color:var(--red);font-size:0.75rem">Erreur : ${u(K(l))}</div>`}}function gt(e){const t=e.linkedin_url?`<a class="profil-linkedin" href="${u(e.linkedin_url)}" target="_blank" rel="noopener noreferrer">↗ LinkedIn</a>`:"",s=[{niveau:"Maîtrisée",cls:"m",items:e.expertises.filter(a=>a.niveau==="Maîtrisée")},{niveau:"Pratiquée",cls:"p",items:e.expertises.filter(a=>a.niveau==="Pratiquée")},{niveau:"Connue",cls:"c",items:e.expertises.filter(a=>a.niveau==="Connue")}].filter(a=>a.items.length>0).map(a=>`
