...
`);b.forEach((h,I)=>a.set(h,$[I]??h))}l.forEach(i=>{const o=_.get(i);i.isConnected&&a.has(o)&&(i.textContent=a.get(o))}),document.documentElement.lang=e,Z=e}catch(i){console.error("Traduction échouée :",i),t.textContent="🌐 ✗",setTimeout(()=>{t.textContent="🌐 LANGUE",t.classList.remove("loading")},2e3);return}t.textContent="🌐 LANGUE",t.classList.remove("loading")};document.addEventListener("click",e=>{!e.target.closest("#trad-btn")&&!e.target.closest("#trad-panel")&&(document.getElementById("trad-panel")?.classList.remove("open"),document.getElementById("trad-btn")?.classList.remove("open"))});window.toggleA11yPanel=function(){const e=document.getElementById("a11y-panel"),t=document.getElementById("a11y-btn"),l=e.classList.toggle("open");t.classList.toggle("open",l)};window.toggleHVMode=function(){const e=document.body.classList.toggle("hv-mode");document.getElementById("hv-switch")?.classList.toggle("on",e),document.getElementById("a11y-hv-btn")?.classList.toggle("active",e),localStorage.setItem("xenna-hv",e?"1":"")};window.toggleZoom=function(){const e=document.body.classList.toggle("zoom-mode");document.documentElement.style.zoom=e?"200%":"",document.getElementById("zoom-switch")?.classList.toggle("on",e),document.getElementById("a11y-magnifier")?.classList.toggle("active",e),localStorage.setItem("xenna-zoom",e?"1":"")};const Q=new Set,Ie=new Set(["IBM Plex Mono","Fira Code","JetBrains Mono","Source Code Pro","Roboto Mono","Inconsolata"]);window.setAppFont=function(e,t=!1){if(!e){document.body.classList.remove("custom-font"),document.documentElement.style.removeProperty("--app-font"),localStorage.removeItem("xenna-font");const a=document.getElementById("font-picker");a&&(a.value="");return}if(!Ie.has(e))return;const l=e.replace(/ /g,"+");if(!Q.has(l)){const a=document.createElement("link");a.rel="stylesheet",a.href=`https://fonts.googleapis.com/css2?family=${l}&display=swap`,document.head.appendChild(a),Q.add(l)}document.documentElement.style.setProperty("--app-font",`'${e}', monospace`),document.body.classList.add("custom-font"),localStorage.setItem("xenna-font",e);const s=document.getElementById("font-picker");s&&t&&(s.value=e)};const L=[];window.scan67=function(){const e=Date.now();for(L.push(e);L.length&&e-L[0]>1500;)L.shift();const t=L.length>=3;t&&(L.length=0);const l=t?/42/:/67/,a=Array.from(document.querySelectorAll(".mob-val, .sb-val, .ascii-tbl td, .fm-val, .fm-result td")).filter(i=>l.test(i.textContent.replace(/[\s ]/g,""))&&i.offsetParent!==null);if(a.length===0)return;const n=document.getElementById("a11y-67-btn");if(n.classList.add("active"),t){const i=["#ff0055","#ff6600","#ffcc00","#00ff88","#00ccff","#aa00ff","#ff00cc","#39ff14","#ff4444","#44ffff","#ff69b4","#7fff00"];a.forEach((o,b)=>{setTimeout(()=>{const c=i[Math.floor(Math.random()*i.length)];Object.assign(o.style,{background:c,color:"#000",outline:`2px solid ${c}`,borderRadius:"2px",transition:"all 0.15s"}),setTimeout(()=>Object.assign(o.style,{background:"",color:"",outline:"",borderRadius:""}),900)},b*250)}),setTimeout(()=>n.classList.remove("active"),a.length*250+1e3)}else a.forEach((i,o)=>{setTimeout(()=>{i.classList.remove("flash-67"),i.offsetWidth,i.classList.add("flash-67"),i.addEventListener("animationend",()=>i.classList.remove("flash-67"),{once:!0})},o*500)}),setTimeout(()=>n.classList.remove("active"),a.length*500+200)};window.toggleBWMode=function(){const e=document.body.classList.toggle("bw-mode");document.getElementById("bw-switch")?.classList.toggle("on",e),document.getElementById("a11y-bw-btn")?.classList.toggle("active",e),localStorage.setItem("xenna-bw",e?"1":"")};function p(e){return String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}window.setView=function(e){["mobile","desktop","annuel","forge","apropos","gaabrielle","hercule"].forEach(t=>document.body.classList.toggle("is-"+t,e===t)),document.getElementById("btn-desk").classList.toggle("active",e==="desktop"),document.getElementById("btn-mob").classList.toggle("active",e==="mobile"),document.getElementById("btn-ann").classList.toggle("active",e==="annuel"),D&&(e==="desktop"||e==="mobile")&&re(D),e==="forge"&&Pe()};let B="EUR";function u(e){const t=parseFloat(e),l=B==="CHF"?" CHF":" €";return t.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+l}function Se(e,t=!1){const l=parseFloat(e),s=B==="CHF"?" CHF":" €",a=l.toLocaleString("fr-FR",{minimumFractionDigits:2,maximumFractionDigits:2})+s;return t&&l>0?"+"+a:a}function T(e){return(parseFloat(e)*100).toFixed(2)+" %"}function ce(){const e=document.body.classList.contains("is-mobile")?"m-date":"d-date",t=e==="m-date"?"d-date":"m-date";return document.getElementById(e)?.value||document.getElementById(t)?.value||oe}function j(e){if(!e)return"—";const[t,l,s]=e.split("-");return`${s}/${l}/${t}`}const we=[{min:0,max:1620,taux:0},{min:1620,max:1683,taux:.005},{min:1683,max:1791,taux:.013},{min:1791,max:1911,taux:.021},{min:1911,max:2042,taux:.029},{min:2042,max:2151,taux:.035},{min:2151,max:2294,taux:.041},{min:2294,max:2714,taux:.053},{min:2714,max:3107,taux:.075},{min:3107,max:3539,taux:.099},{min:3539,max:3983,taux:.119},{min:3983,max:4648,taux:.138},{min:4648,max:5574,taux:.158},{min:5574,max:6974,taux:.179},{min:6974,max:8711,taux:.2},{min:8711,max:12091,taux:.24},{min:12091,max:16376,taux:.28},{min:16376,max:25706,taux:.33},{min:25706,max:55062,taux:.38},{min:55062,max:1/0,taux:.43}];function z(e){const t=parseFloat(e);if(isNaN(t)||t<=0)return{total:0,taux_effectif:0,details:[]};let l=0;const s=[];for(const a of we){if(t<=a.min)break;const i=+((a.max===1/0?t:Math.min(t,a.max))-a.min).toFixed(2),o=i*a.taux;if(s.push({min:a.min,max:a.max===1/0?null:a.max,taux:a.taux,base:i,montant:+o.toFixed(2)}),l+=o,a.max===1/0||t<=a.max)break}return{total:+l.toFixed(2),taux_effectif:t>0?l/t:0,details:s}}const ee={"Sécurité Sociale":"cat-ss","CSG/CRDS":"cat-csg","Retraite complémentaire":"cat-ret",Prévoyance:"cat-prev",Chômage:"cat-cho",Allègement:"cat-alleg","1er pilier":"cat-ss","Assurance chômage":"cat-cho","Assurance accidents":"cat-acc","Prévoyance maladie":"cat-prev","Prévoyance (LPP)":"cat-ret","Assurance pension":"cat-ret","Assurance maladie":"cat-ss","Assurance dépendance":"cat-prev","Mutualité des employeurs":"cat-ss"},F={},te={SS_VIEILLESSE_PLAF:"min(Salaire brut, Plafond Mensuel Sécurité Sociale — PMSS)",CHOMAGE:"min(Salaire brut, 4 × PMSS)",AGS:"min(Salaire brut, 4 × PMSS)",CSG_DEDUCTIBLE:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels (CSS art. L136-2)",CSG_NON_DEDUCTIBLE:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels",CRDS:"Salaire brut × 98,25 %  — abattement forfaitaire frais professionnels",AGIRC_ARRCO_T1:"min(Salaire brut, PMSS)  — Tranche 1 (entre 0 et 1 PMSS)",AGIRC_ARRCO_CEG_T1:"min(Salaire brut, PMSS)  — Tranche 1",PREVOYANCE_CADRE_MIN:"min(Salaire brut, PMSS)  — Tranche A",AGIRC_ARRCO_T2:"Fraction du salaire entre 1 PMSS et 8 PMSS  — Tranche 2",AGIRC_ARRCO_CEG_T2:"Fraction du salaire entre 1 PMSS et 8 PMSS  — Tranche 2"};function H(e,t=!0){const l=["f","(","x",")"].map((s,a)=>`<span style="animation-delay:${a*45}ms">${s}</span>`).join("");return t?`<span class="formula-star" data-fmkey="${e}" onclick="event.stopPropagation();showFormula('${e}')">${l}</span>`:`<span class="formula-star" aria-hidden="true">${l}</span>`}window.togglePasDetail=function(e){const t=document.getElementById(e);if(!t)return;const l=t.style.display!=="none";t.style.display=l?"none":"block";const s=document.getElementById(e+"-arrow");s&&(s.textContent=l?"▶":"▼")};function N(e,t){if(e.code==="REDUCTION_FILLON")return`<pre class="fm-fillon">${p(e.explication)}</pre>`;const l=t==="sal",s=l?e.taux_sal:e.taux_pat,a=parseFloat(e.base),n=l?parseFloat(e.montant_sal):Math.abs(parseFloat(e.montant_pat)),i=l?"Taux salarial":"Taux patronal",o=l?"Montant salarial":t==="alleg"?"Montant allègement":"Montant patronal",b=l?"c-sal":t==="alleg"?"c-alleg":"c-pat",c=te[e.code]?`<div class="fm-base-note">Assiette  =  ${p(te[e.code])}</div>`:"";return`
    ...
    </table>`}function G(e){const t=z(e);return`
    ...
    <div class="summary-bar">
      ...
      <div class="sb-cell">
    ...
    </div>`,c=l.filter(m=>m.categorie!=="Allègement"&&(parseFloat(m.montant_sal)>0||m.taux_sal!=="0"||parseFloat(m.montant_pat)>0)),r=l.filter(m=>m.categorie==="Allègement"),f=c.reduce((m,E)=>m+parseFloat(E.montant_pat),0);function v(m,E){return m.map((g,x)=>{const S=E+x,w=ee[g.categorie]||"cat-ss",q=parseFloat(g.montant_sal)>0?"c-sal":"c-dim",k=parseFloat(g.montant_pat)>0?"c-pat":"c-dim",P=`${g.code}_sal`,J=`${g.code}_pat`,X=parseFloat(g.montant_sal)>0,W=parseFloat(g.montant_pat)>0;X&&(F[P]={c:g,type:"sal"}),W&&(F[J]={c:g,type:"pat"});const ve=H(P,X),ye=H(J,W);return`
        <tr class="data-row" id="row-${S}" onclick="toggleExpl(${S})">
          ...
          <td class="r">${T(g.taux_sal)}</td>
          <td class="r ${q}">${u(g.montant_sal)}${ve}</td>
          <td class="r">${T(g.taux_pat)}</td>
          <td class="r ${k}">${u(g.montant_pat)}${ye}</td>
        </tr>
    ...
    SIMULATION AU <span class="sp-accent">${j(ce())}</span>
    ...
    <table class="ascii-tbl">
      ${$}
      <tbody>
        ${r.map((m,E)=>{const g=c.length+E,x=ee[m.categorie]||"cat-alleg",S=Math.abs(parseFloat(m.montant_pat)),w=`${m.code}_alleg`;return F[w]={c:m,type:"alleg"},`
            <tr class="data-row" id="row-${g}" onclick="toggleExpl(${g})">
              ...
              <td class="r"></td>
              <td class="r"></td>
              <td class="r c-alleg">${T(Math.abs(parseFloat(m.taux_pat)))}</td>
              <td class="r c-alleg">− ${u(S)}${H(w)}</td>
            </tr>
    ...
    </table>`;t.innerHTML=I+b+`<div class="tbl-wrap">${h}${d}</div>`}window.mobToggle=function(e,t){const l=document.getElementById("mob-expand-"+e),s=document.getElementById("mob-expand-"+e+"-why"),a=document.getElementById("mob-expand-"+e+"-how");if(!l)return;const n=l.style.display!=="none",i=l.dataset.panel;n?i===t?l.style.display="none":(l.dataset.panel=t,s.style.display=t==="why"?"block":"none",a.style.display=t==="how"?"block":"none"):(l.style.display="block",l.dataset.panel=t,s.style.display=t==="why"?"block":"none",a.style.display=t==="how"?"block":"none")};function Ce(e,t,l,s,a,n=0){const i=e.code==="REDUCTION_FILLON"?`<pre class="fm-fillon">${p(e.explication)}</pre>`:`<div class="fm-type-${a}">${N(e,a)}</div>`,o=`
    ...
    </div>`}function _e(e){const t=document.getElementById("res-mobile"),l=document.getElementById("m-nom")?.value||document.getElementById("d-nom")?.value||"",s=document.getElementById("m-prenom")?.value||document.getElementById("d-prenom")?.value||"",a=e.cotisations,n=e.salarie?.pays&&e.salarie.pays!=="france",i=a.reduce((d,m)=>d+parseFloat(m.montant_sal),0),o=a.reduce((d,m)=>d+parseFloat(m.montant_pat),0),b=n?{total:0,taux_effectif:0}:z(e.net_imposable),c=parseFloat(e.net_a_payer)-b.total,r=parseFloat(e.brut)+o,f=a.filter(d=>d.categorie!=="Allègement"&&(parseFloat(d.montant_sal)>0||d.taux_sal!=="0"||parseFloat(d.montant_pat)>0)),v=a.filter(d=>d.categorie==="Allègement"),$=f.reduce((d,m)=>d+parseFloat(m.montant_pat),0),h=v.reduce((d,m)=>d+parseFloat(m.montant_pat),0),I=f.map((d,m)=>{const E=parseFloat(d.montant_sal)>0,g=parseFloat(d.montant_pat)>0,x=`${d.code}_u`,S=d.code==="REDUCTION_FILLON"?`<pre class="fm-fillon">${p(d.explication)}</pre>`:[E?`<div class="fm-type-sal">${N(d,"sal")}</div>`:"",g?`<div class="fm-type-pat">${N(d,"pat")}</div>`:""].join(""),w=`
      <div class="mob-exp-txt">${p(d.explication)}</div>
      ${d.loi_ref?`<div class="mob-exp-loi">§ ${p(d.loi_ref)}</div>`:""}`,q=`mob-stripe-sal-${m%2===0?"a":"b"}`,k=E?`<span class="mob-val mob-cot-amt" style="color:#ffe033" onclick="mobToggle('${x}','how')">− ${u(d.montant_sal)}</span>`:`<span class="mob-val c-dim">0 ${B==="CHF"?"CHF":"€"}</span>`,P=g?`<span class="mob-val c-orange mob-cot-amt" onclick="mobToggle('${x}','how')">+ ${u(d.montant_pat)}</span>`:`<span class="mob-val c-dim">0 ${B==="CHF"?"CHF":"€"}</span>`;return`
      <div class="${q}">
        <div class="mob-row">
          <span class="mob-lbl mob-cot-lbl" onclick="mobToggle('${x}','why')">${p(d.libelle)}</span>
          <span style="display:flex;flex-direction:column;align-items:flex-end;gap:0.1rem">${k}${P}</span>
        </div>
        <div class="mob-expand" id="mob-expand-${x}" style="display:none">
          <div id="mob-expand-${x}-why">${w}</div>
          <div id="mob-expand-${x}-how" style="display:none">${S}</div>
        </div>
    ...
    <div class="mob-bulletin">
      ...
      <div class="mob-head">
        ...
        <div style="text-align:right">
          ...
          <div class="mob-head-date">simulation au ${j(ce())}</div>
        </div>
      </div>
